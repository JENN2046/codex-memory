...
function preconditionFailureReason(error) {
...
function acquisitionFailureReason(error) {
  ...
  return preconditionFailureReason(error);
}
...
function projectAcquisitionFailure(error, context) {
  ...
  const cleanupFailed = structured &&
  ...
  const lifecycleLockErrorCode = cleanupFailed
    ? optionalStableCode(error.cleanupErrorCode)
    : structured
      ? null
      : isLifecycleLockErrorCode(rawErrorCode)
        ? rawErrorCode
        : null;
  const preconditionErrorCode = cleanupFailed
    ? optionalStableCode(error.primaryErrorCode)
    : structured
      ? optionalStableCode(error.primaryErrorCode || rawErrorCode)
      : lifecycleLockErrorCode === null
        ? rawErrorCode
        : null;
  return emptyProjection({
    ...
    stableReason: cleanupFailed
      ...
      : structured
        ? acquisitionFailureReason(error)
        : lifecycleLockErrorCode !== null
...
function projectP1Result(transaction, context) {
  ...
  return emptyProjection({
    ...
    profileTransactionErrorCode: optionalStableCode(transaction.errorCode),
    ...
    currentProfileFingerprint: transaction.oldFingerprint ||
      context.currentProfileFingerprint || null,
...
function projectReleaseFailure(previous, releaseError, context) {
  const underlyingClassification = previous?.classification ||
    'PRECONDITION_REJECTED';
  return emptyProjection({
    ...previous,
    ...context,
...
function assertCandidateSource(source, binding, { identity = null } = {}) {
...
function coordinateStoppedOwnerProfileTransition({
  ...
  try {
    ...
    const nextProfile = buildNextProfile(current, binding, validateProfile);
