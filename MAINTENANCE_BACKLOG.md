...
| P52-T2 | memory-governance | A2 | todo | Minimal Runtime Enforcement Helper | changed JS syntax; targeted helper tests; `npm test`; `git diff --check`; no-touch/boundary scan | Next safe slice: pure explicit-input helper over caller-provided object only; no fs read, command execution, durable write, public MCP expansion, provider call, or real memory read. |
