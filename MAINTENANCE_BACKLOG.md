...
| P16.5-compare-rollback-semantic-gate | tagmemo / semantic-association / compare-rollback | A1/A2 | todo | Run or summarize compare/rollback semantic evidence before P16 closeout | targeted TagMemo tests; ordering compare/rollback when applicable; `npm test`; diff/docs validation | Must not force misleading compare category if semantic surface is not covered; no runtime tuning unless later approved |
...
67. 下一步建议在 P16.4 validation/commit/push 后进入 `P16.5-compare-rollback-semantic-gate`，先收 compare/rollback 证据；不得直接进入 runtime implementation、provider benchmark、V8、UI、migration/import-export apply 或 release candidate。
