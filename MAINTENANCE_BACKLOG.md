...
| P13.7-migration-readiness-report | memory-governance / object-model | A2 | todo | Add read-only migration readiness report surface | future readiness CLI test; readiness JSON smoke; `npm test`; diff/docs validation | Must report `migrationBlocked=true` and `mutated=false`; no migration, DB/diary write, import/export apply, or MCP expansion |
...
44. 下一步建议：P13.7 migration readiness report。
45. provider/profile 相关动作继续保持按需触发，除非用户明确要求，不主动跑真实 provider 命令。
