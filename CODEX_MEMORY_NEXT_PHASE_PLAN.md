...
- Recommended next local phase: P66.21 ValidationAggregator missing or stale evidence fail-closed helper. 中文解释：下一步只能新增纯 explicit-input helper 来校验 caller-provided missing/stale evidence metadata；不能读取文件、执行命令、启动服务、调用 provider、写 durable state 或声明 `RC_READY`。
