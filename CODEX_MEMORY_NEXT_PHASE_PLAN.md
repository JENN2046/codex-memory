...
- Recommended next local phase: P66.20 ValidationAggregator missing or stale evidence fail-closed proof. 中文解释：下一步只能定义或加固缺失/陈旧 evidence 的 fail-closed 本地证明；不能读取真实 evidence 文件、执行 runtime/gate/runner、启动服务、调用 provider、写 durable state 或声明 `RC_READY`。
