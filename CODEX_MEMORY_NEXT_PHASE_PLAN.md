...
After P60-T1 commit `66d1978`:

- Reconcile active board/status to `66d1978`.
- Consider P61-T1 RC evidence report boundary inventory only if it remains docs/fixture/test or explicit-input report-shape work and does not execute mainline gate, live HTTP operation, provider calls, service/watchdog/startup install, config switch, durable write, public MCP expansion, push/tag/release/deploy, or RC-ready claim.
