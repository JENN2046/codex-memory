...
- Next recommended phase: `P31.3-memory-governance-lifecycle-contract-helper-candidate`. It may plan or implement a pure explicit-input helper over a caller-provided governance lifecycle fixture object only. Real runtime enforcement, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
