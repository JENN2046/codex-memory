...
- Next recommended phase: `P32-memory-governance-approval-packet-safe-scope-inventory`. It may define a future approval packet for runtime governance actions using docs/fixture-first inputs only. Durable mutation, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
