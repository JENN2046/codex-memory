...
- 下一步建议：request explicit GitHub release approval for `p22-rc-7fd17de` if publication should continue; old `p22-rc-806cc847` remains superseded and must not be moved/reused; deploy / provider / config / migration-import-export / public MCP expansion still requires separate explicit approval.
