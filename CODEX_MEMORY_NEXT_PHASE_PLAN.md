...
- Next recommended phase: `P30.2-final-rc-validation-matrix-runner-manifest-helper`. It may add a pure helper over explicit fixture input only. Real runtime enforcement, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
