...
- Next recommended phase: `P31-memory-governance-safe-scope-inventory`. It should stay docs/fixture-first and define proposal, supersession, tombstone, validation, audit, and approval boundaries without mutating durable memory. Real runtime enforcement, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
