...
- Next recommended phase: `P32.2-memory-governance-approval-packet-helper-candidate`. It may add a pure explicit-input helper for caller-provided approval packet objects. Durable mutation, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
