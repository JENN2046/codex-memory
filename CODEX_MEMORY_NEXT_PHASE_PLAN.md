...
- Recommended next local phase: P66.25 ValidationAggregator unsupported source fail-closed helper. 中文解释：下一步只能新增纯 explicit-input helper 来校验 caller-provided unsupported source metadata，并保持 no-touch；不能读取真实 evidence 文件、执行命令、启动服务、调用 provider、写 durable state 或声明 `RC_READY`。
