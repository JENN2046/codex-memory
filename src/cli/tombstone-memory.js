...
function parseArgs(argv = []) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (VALUE_FLAGS.has(token)) {
      ...
      if (token === '--tombstone-reason') options.tombstoneReason = value;
...
function validateCliOptions(options) {
  ...
  if (options.tool && options.tool !== 'memory_tombstone') {
  ...
  if (options.workspaceId) {
    return 'raw workspace_id is not accepted by tombstone-memory CLI output.';
  }
  ...
  if (!options.tombstoneReason) return 'missing tombstone-reason.';
...
function buildServicePayload(options) {
  return {
    ...
    tombstone_reason: options.tombstoneReason,
...
function buildFallbackAuditPreview(result, options) {
  return {
    ...
    to_status: result.toStatus || 'tombstoned',
    created_at: null,
    reversible: true,
...
function buildReportFromResult(result, options) {
  const auditPreview = summarizeAuditEvent(result.auditEventPreview || result.auditEventIntent || null)
  ...
  return {
    ...
    toStatus: result.toStatus || 'tombstoned',
...
async function buildReport(options, appFactory = createCodexMemoryApplication) {
  ...
  try {
    const result = await app.services.tombstoneMemoryService.tombstone(buildServicePayload(options));
...
async function main() {
  ...
  try {
    ...
    output = {
      report: buildCliRejection(error.message || 'tombstone-memory CLI failed.', options),
      exitCode: 1
    };
  }
