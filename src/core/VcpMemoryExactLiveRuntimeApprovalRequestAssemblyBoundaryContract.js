...
const CONTRACT_NAME = 'VcpMemoryExactLiveRuntimeApprovalRequestAssemblyBoundaryContract';
const CONTRACT_MODE = 'fixture_exact_live_runtime_approval_request_assembly_boundary_only';
const SCHEMA_VERSION = 1;
const CONTRACT_VERSION = 'vcp_memory_exact_live_runtime_approval_request_assembly_boundary_v1';
const BOUNDARY_ID_PREFIX = 'cm1878_fixture_exact_live_runtime_approval_request_assembly_boundary_';

const ALLOWED_EVIDENCE_TYPES = Object.freeze(['fixture-only', 'schema-only']);
const ALLOWED_PROFILES = Object.freeze(['exact-live-runtime-approval-request-assembly-boundary']);
...
const REQUIRED_SOURCE_FIELDS = Object.freeze([
  ...
  'cm1875_assembly_preflight_fixture_contract_present',
  'cm1876_assembly_preflight_closeout_present',
  'cm1877_assembly_boundary_review_present',
  'validation_cmv1980_present'
]);

const REQUIRED_BOUNDARY_FIELDS = Object.freeze([
  'field_categories_declared',
...
function lowDisclosureProjection(input) {
  ...
  return {
    ...
    cm1877AssemblyBoundaryReviewPresent: isPlainObject(sources)
      ? sources.cm1877_assembly_boundary_review_present === true
      : false,
    validationCmv1980Present: isPlainObject(sources)
      ? sources.validation_cmv1980_present === true
      : false,
...
function sideEffectPosture(computedDecision = null) {
  return {
    assemblyBoundaryAccepted:
      computedDecision === 'assembly_boundary_accepted_category_only_non_authorizing',
...
function isBoundaryComplete(boundary) {
  return (
    boundary.field_categories_declared === true &&
...
function computeDecision(input) {
  ...
  if (!isSourceChainComplete(input.sources)) return 'assembly_boundary_incomplete';
  if (!isBoundaryComplete(input.boundary)) return 'assembly_boundary_incomplete';
  return 'assembly_boundary_accepted_category_only_non_authorizing';
}

function validateVcpMemoryExactLiveRuntimeApprovalRequestAssemblyBoundaryContract(input) {
  ...
  if (invalidFields.length > 0) {
    return rejected('invalid_exact_live_runtime_approval_request_assembly_boundary_contract', input, {
      invalidFields
    });
  }
  ...
  return {
    ...
    forbiddenCounters: [],
    invalidFields: [],
    nextAction: 'assembly_boundary_validated_category_only_no_authority',
