...
const CONTRACT_NAME = 'Phase2NativeTargetBindingReceiptReviewContract';
const CONTRACT_MODE = 'local_phase2_native_target_binding_receipt_review_only';
...
const REQUIRED_PREREQUISITES = Object.freeze([
  ...
  'completionAuditStillRequiresNativeTargetBindingPassed',
  'localReviewDoesNotSatisfyNativeTargetBinding'
]);
...
const REQUIRED_RECEIPT_FIELD = 'nativeTargetBindingPassed';
const REQUIRED_RECEIPT_CATEGORY = 'nativeTargetBindingReceipt';
...
function invalidTargetBindingReview(review) {
  if (!isPlainObject(review)) return ['targetBindingReview'];
  const blockers = [];

  blockers.push(...missingFields(review, REQUIRED_REVIEW_FIELDS, 'targetBindingReview'));

  if (review.reviewPrepared !== true) blockers.push('targetBindingReview.reviewPrepared');
  if (review.receiptField !== REQUIRED_RECEIPT_FIELD) blockers.push('targetBindingReview.receiptField');
  if (review.requiredReceiptCategory !== REQUIRED_RECEIPT_CATEGORY) {
    blockers.push('targetBindingReview.requiredReceiptCategory');
  }
  if (!isSafeReferenceName(review.targetReferenceName)) {
    blockers.push('targetBindingReview.targetReferenceName');
  }
  if (review.safeReferenceNameCategory !== REQUIRED_SAFE_REFERENCE_CATEGORY) {
    blockers.push('targetBindingReview.safeReferenceNameCategory');
  }
  if (review.targetBindingObservedCategory !== REQUIRED_EVIDENCE_MARKER) {
    blockers.push('targetBindingReview.targetBindingObservedCategory');
  }
  for (const field of ['categoryOnly', 'lowDisclosureOnly']) {
    if (review[field] !== true) blockers.push(`targetBindingReview.${field}`);
  }
  for (const field of [
    'endpointLocatorIncluded',
    'targetValueIncluded',
    ...
    if (review[field] !== false) blockers.push(`targetBindingReview.${field}`);
  }
...
function invalidProposedCompletionEvidence(evidence) {
  ...
  const blockers = [];
  if (evidence.nativeTargetBindingPassed !== REQUIRED_EVIDENCE_MARKER) {
    blockers.push('proposedCompletionEvidence.nativeTargetBindingPassed');
  }
  if (evidence.phase2NativeTargetBindingReceiptReviewPassed !== true) {
    blockers.push('proposedCompletionEvidence.phase2NativeTargetBindingReceiptReviewPassed');
  }
...
function failure(reasonCode, extras = {}) {
  return {
    ...
    forbiddenFields: [],
    stopReasons: [],
    phase2NativeTargetBindingReceiptReviewPassed: false,
    nativeTargetBindingPassed: false,
...
function evaluatePhase2NativeTargetBindingReceiptReviewContract(input = {}) {
  ...
  const blockers = [
    ...
    ...invalidTargetBindingReview(input.targetBindingReview),
  ...
  return {
    ...
    contractMode: CONTRACT_MODE,
    decision: accepted
      ? 'phase2_native_target_binding_receipt_review_accepted'
      : 'phase2_native_target_binding_receipt_review_incomplete',
    blockers: sortedUnique(blockers),
    forbiddenFields: [],
    stopReasons: [],
    phase2NativeTargetBindingReceiptReviewPassed: accepted,
    nativeTargetBindingPassed: false,
    ...
    receiptReviewBoundary: {
      ...
      targetReferenceName: isSafeReferenceName(input.targetBindingReview?.targetReferenceName)
        ? input.targetBindingReview.targetReferenceName
        : null,
      exactAuthorizedReceiptStillRequired: true,
      localReviewSatisfiesNativeTargetBindingPassed: false,
      endpointLocatorDisclosed: false,
      targetValueDisclosed: false,
      responseBodyConsumed: false,
      approvalLineAccepted: false,
      callsRuntime: false,
      performsNativeTargetBinding: false,
    ...
    fullPlanPackCompleted: false,
    nextGate: accepted
      ? 'collect_exact_authorized_native_target_binding_receipt_before_marking_nativeTargetBindingPassed'
      : 'repair_native_target_binding_receipt_review_without_runtime_or_raw_target_values'
  };
}
