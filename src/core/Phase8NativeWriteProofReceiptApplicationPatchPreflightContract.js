...
} = require('./Phase8NativeWriteProofReceiptBundleContract');

const CONTRACT_NAME = 'Phase8NativeWriteProofReceiptApplicationPatchPreflightContract';
const CONTRACT_MODE = 'local_phase8_native_write_receipt_application_patch_preflight_only';
const SCHEMA_VERSION = 1;

const ALLOWED_MODES = Object.freeze(['local-native-write-receipt-application-patch-preflight']);
const ALLOWED_DECISIONS = Object.freeze([
  'phase8_native_write_receipt_application_patch_preflight_ready_for_future_exact_receipts',
  'phase8_native_write_receipt_application_patch_preflight_blocked',
  'stop_l4'
]);
...
const REQUIRED_PREREQUISITE_FIELDS = Object.freeze([
  ...
  'cm2027ReceiptAuditIntakeAccepted',
  'cm2029ReceiptBundleContractAccepted',
  'cm2017CompletionAuditRequiresPhase8Proof',
  'cm2024TraceMatrixRequiresExactReceiptEvidence',
  'phase8StillIncompleteBeforePatch',
...
const PHASE8_PATCH_EVIDENCE_FIELDS = Object.freeze([
...
const REQUIRED_RECEIPT_BUNDLE_DECISION =
  'phase8_native_write_receipt_bundle_contract_ready_for_future_evidence';
...
const STOP_L4_TRUE_FIELDS = Object.freeze([
  ['receiptBundleContract', 'currentPhase8Completed'],
  ['receiptBundleContract', 'fullPlanPackCompleted'],
  ...
  ['receiptBundleContract', 'productionWriteProven'],
  ['receiptBundleContract', 'realRootDurableWriteProven'],
  ['receiptBundleContract', 'verifyWriteExecuted'],
  ['receiptBundleContract', 'rollbackDrillExecuted'],
  ['receiptBundleContract', 'failureRecoveryExecuted'],
  ...
  ['patchPreflight', 'phase8CompletionClaimed'],
  ['patchPreflight', 'productionWriteClaimed'],
...
function collectUnexpectedFields(input) {
  ...
  return [
    ...
    ...collectUnexpectedKeys(input.proposedPatchEvidence, PHASE8_PATCH_EVIDENCE_FIELDS, 'proposedPatchEvidence'),
...
function invalidEvidenceMarkers(proposedPatchEvidence) {
  if (!isPlainObject(proposedPatchEvidence)) return [...PHASE8_PATCH_EVIDENCE_FIELDS];
  return PHASE8_PATCH_EVIDENCE_FIELDS
...
function invalidReceiptBundlePrerequisiteSummary(receiptBundleContract) {
  ...
  return REQUIRED_BUNDLE_PREREQUISITE_FIELDS
...
function computeDecision(input) {
  ...
  if (blockers.length > 0) {
    return {
      decision: 'phase8_native_write_receipt_application_patch_preflight_blocked',
      blockers
    };
  }

  return {
    decision: 'phase8_native_write_receipt_application_patch_preflight_ready_for_future_exact_receipts',
    blockers: []
  };
}
...
function evaluatePhase8NativeWriteProofReceiptApplicationPatchPreflightContract(input) {
  ...
  const missing = [
    ...
    ...missingFields(PHASE8_PATCH_EVIDENCE_FIELDS, input.proposedPatchEvidence, 'proposedPatchEvidence'),
  ...
  return {
    ...
    requiredPatchEvidenceFields: [...PHASE8_PATCH_EVIDENCE_FIELDS],
    proposedCompletionAuditEvidence: accepted
      ? PHASE8_PATCH_EVIDENCE_FIELDS.map(field => ({
        field,
    ...
    nextGate: accepted
      ...
      : 'repair_phase8_receipt_application_patch_preflight',
    approvalAcceptedByThisContract: false,
    receiptBundleAppliedToCompletionAudit: false,
    completionAuditPatchApplied: false,
    nativeWriteExecuted: false,
