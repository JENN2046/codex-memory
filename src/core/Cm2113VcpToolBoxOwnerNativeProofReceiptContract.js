...
function evaluateCm2113VcpToolBoxOwnerNativeProofReceipt(receipt = {}) {
  ...
  for (const field of ['vcpToolBoxOwnedRuntimeWritePassed', 'actualTransportBindingPassed', 'stableTargetStoreIdentityPassed']) if (receipt.completionEvidence?.[field] !== true) blockers.push(`receipt.completionEvidence.${field}`);
  for (const field of ['derivedIndexProofAccepted', 'productionProviderProofAccepted', 'productionReady', 'releaseReady', 'rcReady', 'completeV8', 'fullPlanPackCompleted', 'readinessClaimed']) if (receipt.nonClaims?.[field] !== false) blockers.push(`receipt.nonClaims.${field}`);
