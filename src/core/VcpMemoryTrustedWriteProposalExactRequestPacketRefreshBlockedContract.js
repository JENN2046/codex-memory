...
const CONTRACT_NAME = 'VcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract';
const CONTRACT_MODE = 'fixture_trusted_write_proposal_exact_request_packet_refresh_blocked_only';
const SCHEMA_VERSION = 1;

const PACKET_REFRESH_CONTRACT_VERSION =
  'vcp_memory_trusted_write_proposal_exact_request_packet_refresh_blocked_v1';
const PACKET_REFRESH_ID_PREFIX =
  'm9_fixture_trusted_write_proposal_exact_request_packet_refresh_blocked_';
...
const ALLOWED_DISCLOSURE_LEVELS = Object.freeze([
  'redacted_packet_refresh',
  'field_names_only',
  'none'
]);
const ALLOWED_NEXT_ACTIONS = Object.freeze([
  'cm1840_fixture_contract',
...
const REQUIRED_PACKET_FIELDS = Object.freeze([
  ...
  'profile',
  'non_authorizing',
  'exact_request_packet_refresh_blocked_only'
]);
...
const REQUIRED_EVIDENCE_FIELDS = Object.freeze([
  ...
  'cm1828_field_candidate_preflight_present',
  'cm1831_packet_readiness_contract_present',
  ...
  'cm1839_blocked_preflight_present',
  'local_approval_request_readiness_blocked_slice_closed'
]);

const REQUIRED_REFRESH_FIELDS = Object.freeze([
  'local_packet_refresh_blocked_fixture_present',
...
const FORBIDDEN_FIELD_NAMES = Object.freeze([
  ...
  'exactRequestPacketRefreshReady',
  'packetRefreshReady',
  'rawAuditRow',
  'rawSqliteRow',
  'rawJsonlRow',
...
function collectUnexpectedFields(input) {
  ...
  return [
    ...
    ...collectUnexpectedKeys(input.refresh, REQUIRED_REFRESH_FIELDS, 'refresh'),
...
function safeRefreshId(input) {
  const refreshId = isPlainObject(input?.packet) ? input.packet.refresh_id : null;
  return isFixtureIdentifier(refreshId, PACKET_REFRESH_ID_PREFIX) ? refreshId : null;
}

function lowDisclosureProjection(input) {
  const evidence = isPlainObject(input) ? input.evidence : null;
  const refresh = isPlainObject(input) ? input.refresh : null;
  return {
    expectedDecision: safeExpectedDecision(input),
    refreshId: safeRefreshId(input),
    cm1839BlockedPreflightPresent: isPlainObject(evidence)
      ? evidence.cm1839_blocked_preflight_present === true
      : false,
    localApprovalRequestReadinessBlockedSliceClosed: isPlainObject(evidence)
      ? evidence.local_approval_request_readiness_blocked_slice_closed === true
      : false,
    localPacketRefreshBlockedFixturePresent: isPlainObject(refresh)
      ? refresh.local_packet_refresh_blocked_fixture_present === true
      : false,
    exactRequestPacketReady: isPlainObject(refresh)
      ? refresh.exact_request_packet_ready === true
      : false,
    approvalRequestReady: isPlainObject(refresh)
      ? refresh.approval_request_ready === true
      : false
  };
}
...
function sideEffectPosture(computedDecision = null) {
  return {
    exactRequestPacketRefreshAccepted:
      computedDecision === 'exact_request_packet_refresh_blocked_missing_exact_fields',
    nonAuthorizingPacketRefreshBlockedOnly: true,
...
function validateShape(input) {
  ...
  if (!isFixtureIdentifier(input.packet.refresh_id, PACKET_REFRESH_ID_PREFIX)) {
    invalid.push('packet.refresh_id');
  }
  if (input.packet.contract_version !== PACKET_REFRESH_CONTRACT_VERSION) {
  ...
  if (input.packet.exact_request_packet_refresh_blocked_only !== true) {
    invalid.push('packet.exact_request_packet_refresh_blocked_only');
  }
  ...
  invalid.push(...validateBooleanFields(input.refresh, REQUIRED_REFRESH_FIELDS, 'refresh'));
...
function hasL4Stop(input) {
  ...
  return (
    refresh.exact_request_packet_ready ||
    refresh.exact_request_packet_present ||
    refresh.approval_request_ready ||
    refresh.concrete_values_present ||
    refresh.exact_target_bound ||
    refresh.exact_transport_bound ||
    refresh.exact_client_ids_bound ||
    refresh.exact_workspace_scope_bound ||
    refresh.exact_owner_scope_bound ||
    refresh.exact_visibility_bound ||
    refresh.exact_proposal_scope_bound ||
    refresh.exact_proposal_operation_bound ||
    refresh.exact_payload_shape_bound ||
    refresh.exact_review_route_bound ||
    refresh.exact_rollback_posture_bound ||
    refresh.exact_budgets_bound ||
    refresh.l4_write_intent_shield_proven ||
    refresh.real_proposal_receipt_audit_bound ||
    refresh.approval_request_submission_authority_bound ||
    refresh.approval_line_value_present ||
    refresh.approval_request_body_prepared ||
...
function isBlockedPacketRefreshStateDeclared(refresh, missing) {
  return (
    refresh.local_packet_refresh_blocked_fixture_present === true &&
    refresh.accepted_planning_evidence_present === true &&
    refresh.exact_request_packet_ready === false &&
    refresh.exact_request_packet_present === false &&
    refresh.approval_request_ready === false &&
    refresh.concrete_values_present === false &&
    refresh.approval_request_body_prepared === false &&
    refresh.missing_exact_fields_declared === true &&
...
function computeDecision(input) {
  ...
  if (!isEvidenceComplete(input.evidence)) return 'exact_request_packet_refresh_incomplete';
  if (!isBlockedPacketRefreshStateDeclared(input.refresh, input.missingExactFields)) {
    return 'exact_request_packet_refresh_incomplete';
  }
  return 'exact_request_packet_refresh_blocked_missing_exact_fields';
}

function validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(input) {
  ...
  const missing = [
    ...
    ...missingFields(REQUIRED_REFRESH_FIELDS, input.refresh, 'refresh'),
  ...
  if (invalidFields.length > 0) {
    return rejected('invalid_trusted_write_proposal_exact_request_packet_refresh_blocked_contract', input, {
      invalidFields
    });
  }
  ...
  return {
    ...
    packetRefreshContractVersion: PACKET_REFRESH_CONTRACT_VERSION,
    refreshId: input.packet.refresh_id,
    ...
    forbiddenCounters: [],
    invalidFields: [],
    nextAction: 'packet_refresh_validated_blocked_no_authority',
...
module.exports = {
  ...
  PACKET_REFRESH_CONTRACT_VERSION,
  PACKET_REFRESH_ID_PREFIX,
