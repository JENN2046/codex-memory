...
function buildRc9MarkdownAudit(markdown = '') {
  const renderedMarkdown = typeof markdown === 'string' ? markdown : '';
  const markdownAuditSections = [
    ...
    const missingFragments = section.requiredFragments
      .filter(fragment => !renderedMarkdown.includes(fragment));
    return {
      id: section.id,
...
function renderRc9DecisionPacketFromAggregatorReport(report = null, options = {}) {
  ...
  const markdown = [
    ...
    `route_approval_hint_count = ${packet.routeApprovalHintAudit.approvalHintCount}`,
    `route_approval_hint_missing_count = ${packet.routeApprovalHintAudit.approvalHintMissingCount}`,
    `route_approval_hint_manual_review_count = ${packet.routeApprovalHintAudit.manualReviewApprovalHintCount}`,
    `route_approval_hint_approval_generated = ${packet.routeApprovalHintAudit.approvalGenerated}`,
    `route_approval_hint_approval_accepted = ${packet.routeApprovalHintAudit.approvalAccepted}`,
    `route_approval_hint_approval_executed = ${packet.routeApprovalHintAudit.approvalExecuted}`,
    `route_approval_hint_can_claim_readiness = ${packet.routeApprovalHintAuditCanClaimReadiness}`,
    '',
    '## Remaining Gaps',
    '',
  ...
  const markdownAudit = buildRc9MarkdownAudit(markdown);
