...
const CONTRACT_NAME = 'Phase8ExactReceiptRequestBoundaryContract';
const CONTRACT_MODE = 'local_phase8_exact_receipt_request_boundary_only';
const SCHEMA_VERSION = 1;

const PHASE8_REQUIREMENT_ID = 'phase8_native_write_production_proof';
const PHASE8_EXACT_RECEIPT_FIELDS = Object.freeze(
  [...EXACT_RECEIPT_REQUIREMENTS[PHASE8_REQUIREMENT_ID]]
);

const ALLOWED_MODES = Object.freeze(['local-phase8-exact-receipt-request-boundary']);
const ALLOWED_DECISIONS = Object.freeze([
  'phase8_exact_receipt_request_boundary_prepared',
  'phase8_exact_receipt_request_boundary_blocked',
  'stop_l4'
]);
...
const REQUIRED_REQUEST_BOUNDARY_FIELDS = Object.freeze([
  ...
  'memoryWriteExecuted',
  'durableMemoryWriteExecuted',
  'auditRowsRead',
  'outputDisclosureMaterialRead',
...
const COUNTER_FIELDS = Object.freeze([
  ...
  'memoryWrites',
  'durableMemoryWrites',
  'verifyWriteExecutions',
  'rollbackExecutions',
  'failureRecoveryExecutions',
  'auditRowReads',
  'outputDisclosureReads',
...
const REQUEST_STOP_TRUE_FIELDS = Object.freeze([
  ...
  ['requestBoundary', 'nativeWriteExecuted'],
  ['requestBoundary', 'productionWriteProven'],
  ['requestBoundary', 'realRootDurableWriteProven'],
  ['requestBoundary', 'verifyWriteExecuted'],
  ['requestBoundary', 'rollbackDrillExecuted'],
  ['requestBoundary', 'failureRecoveryExecuted'],
  ['requestBoundary', 'memoryWriteExecuted'],
  ['requestBoundary', 'durableMemoryWriteExecuted'],
  ['requestBoundary', 'auditRowsRead'],
  ['requestBoundary', 'outputDisclosureMaterialRead'],
  ['requestBoundary', 'localContractsAllowedToSatisfyExactReceipts'],
  ['requestBoundary', 'phase8CompletionClaimed'],
  ['requestBoundary', 'productionWriteClaimed']
]);
...
function phase8ExactReceiptFieldsFromRoute(routeResult = {}) {
  ...
  return exactRoutes
    ...
    .filter(item => item.scope === 'phase' && item.requirementId === PHASE8_REQUIREMENT_ID)
    .map(item => item.evidenceField)
    .filter(field => PHASE8_EXACT_RECEIPT_FIELDS.includes(field));
}

function missingPhase8ExactReceiptFields(routeResult) {
  const routed = new Set(phase8ExactReceiptFieldsFromRoute(routeResult));
  return PHASE8_EXACT_RECEIPT_FIELDS.filter(field => !routed.has(field));
}
...
function validateRequestBoundary(requestBoundary) {
  return [
    ...
    requestBoundary.phaseId === PHASE8_REQUIREMENT_ID ? null : 'requestBoundary.phaseId'
  ].filter(Boolean);
}
...
function computeDecision(input) {
  ...
  const missingPhase8Fields = missingPhase8ExactReceiptFields(input.routeResult);
  for (const field of missingPhase8Fields) {
    blockers.push(`phase8ExactReceiptRoute.${field}`);
  }

  if (blockers.length > 0) {
    return { decision: 'phase8_exact_receipt_request_boundary_blocked', blockers };
  }

  return { decision: 'phase8_exact_receipt_request_boundary_prepared', blockers: [] };
}

function futureReceiptRequestEntries() {
  return PHASE8_EXACT_RECEIPT_FIELDS.map(evidenceField => ({
...
function evaluatePhase8ExactReceiptRequestBoundaryContract(input) {
  ...
  if (computed.decision !== 'phase8_exact_receipt_request_boundary_prepared') {
    return {
      ...failure('phase8_exact_receipt_request_boundary_not_ready'),
      decision: computed.decision,
      blockers: computed.blockers
    };
  }
  ...
  return {
    ...
    nextGate: 'await_separate_jenn_exact_authorization_before_phase8_receipt_collection_or_application',
    receiptAcceptedByThisContract: false,
    receiptAppliedByThisContract: false,
    currentPhase8Completed: false,
    fullPlanPackCompleted: false,
