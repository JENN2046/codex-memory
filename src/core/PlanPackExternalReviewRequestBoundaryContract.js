...
const CONTRACT_NAME = 'PlanPackExternalReviewRequestBoundaryContract';
const CONTRACT_MODE = 'local_plan_pack_external_review_request_boundary_only';
const SCHEMA_VERSION = 1;

const PHASE9_REQUIREMENT_ID = 'phase9_default_runtime_policy';
const PHASE10_REQUIREMENT_ID = 'phase10_tag_release_readiness';

const REQUIRED_EXTERNAL_REVIEW_ROUTE_KEYS = Object.freeze([
  Object.freeze({
    scope: 'phase',
    requirementId: PHASE9_REQUIREMENT_ID,
  ...
  Object.freeze({
    scope: 'phase',
    requirementId: PHASE10_REQUIREMENT_ID,
    evidenceField: 'externalReviewPassed'
  }),
  Object.freeze({
    scope: 'phase',
...
const ALLOWED_MODES = Object.freeze(['local-plan-pack-external-review-request-boundary']);
const ALLOWED_DECISIONS = Object.freeze([
  'plan_pack_external_review_request_boundary_prepared',
  'plan_pack_external_review_request_boundary_blocked',
  'stop_l4'
]);
...
const REQUIRED_REQUEST_BOUNDARY_FIELDS = Object.freeze([
  'boundaryPrepared',
  'externalReviewEvidenceRequired',
  'observationOrDogfoodReviewRequired',
  'externalReviewRequired',
  'tagApprovalPacketRequired',
  'lowDisclosureOnly',
  'categoryOnly',
  'separateJennApprovalRequired',
  'reviewAcceptedByThisContract',
  'tagApprovalAcceptedByThisContract',
  'reviewBundleAcceptedByThisContract',
...
const REQUEST_STOP_TRUE_FIELDS = Object.freeze([
  ['requestBoundary', 'reviewAcceptedByThisContract'],
  ['requestBoundary', 'tagApprovalAcceptedByThisContract'],
  ['requestBoundary', 'reviewBundleAcceptedByThisContract'],
  ['requestBoundary', 'reviewAppliedByThisContract'],
  ['requestBoundary', 'completionAuditPatchApplied'],
  ['requestBoundary', 'defaultRuntimeExpanded'],
  ['requestBoundary', 'tagCreated'],
  ['requestBoundary', 'tagPushed'],
  ['requestBoundary', 'releasePublished'],
  ['requestBoundary', 'deploymentTriggered'],
  ['requestBoundary', 'cutoverPerformed'],
  ['requestBoundary', 'localContractsAllowedToSatisfyExternalReview'],
  ['requestBoundary', 'phase9CompletionClaimed'],
  ['requestBoundary', 'phase10CompletionClaimed'],
  ['requestBoundary', 'releaseReadinessClaimed'],
  ['requestBoundary', 'fullPlanPackCompletionClaimed']
]);
...
function routeKey(item) {
  return `${item.scope}:${item.requirementId}:${item.evidenceField}`;
}
...
function externalReviewRouteItemsFromRoute(routeResult = {}) {
  const summary = isPlainObject(routeResult.routeSummary) ? routeResult.routeSummary : {};
  const externalRoutes = Array.isArray(summary.external_review_required)
    ? summary.external_review_required
    : [];

  return externalRoutes
    .filter(item => isPlainObject(item))
    .filter(item => item.scope === 'phase')
    .filter(item => [PHASE9_REQUIREMENT_ID, PHASE10_REQUIREMENT_ID].includes(item.requirementId))
    .filter(item => EXTERNAL_REVIEW_FIELDS.includes(item.evidenceField))
    .map(item => ({
      scope: item.scope,
...
function missingExternalReviewRouteKeys(routeResult) {
  const routed = new Set(externalReviewRouteItemsFromRoute(routeResult).map(routeKey));
  return REQUIRED_EXTERNAL_REVIEW_ROUTE_KEYS
    .filter(item => !routed.has(routeKey(item)))
    .map(item => routeKey(item));
}
...
function validateRouteResult(routeResult) {
  ...
  if (
    ...
    !Number.isInteger(routeResult.routeCounts.external_review_required) ||
    routeResult.routeCounts.external_review_required <= 0
  ) {
    blockers.push('routeResult.routeCounts.external_review_required');
  }
...
function computeDecision(input) {
  ...
  const blockers = [
    ...
    ...missingExternalReviewRouteKeys(input.routeResult).map(key => `externalReviewRoute.${key}`)
  ];

  if (blockers.length > 0) {
    return { decision: 'plan_pack_external_review_request_boundary_blocked', blockers };
  }

  return { decision: 'plan_pack_external_review_request_boundary_prepared', blockers: [] };
}

function reviewEvidenceKind(evidenceField) {
  if (evidenceField === 'observationOrDogfoodReviewPassed') return 'future_observation_or_dogfood_review';
  if (evidenceField === 'tagApprovalPacketPassed') return 'future_tag_approval_packet';
  if (evidenceField === 'externalReviewEvidenceBundleAppliedToCompletionAudit') {
    return 'future_external_review_bundle_application_receipt';
  }
  return 'future_external_review';
}

function futureReviewRequestEntries() {
  return REQUIRED_EXTERNAL_REVIEW_ROUTE_KEYS.map(item => ({
    phaseId: item.requirementId,
    evidenceField: item.evidenceField,
    requiredEvidenceKind: reviewEvidenceKind(item.evidenceField),
...
function evaluatePlanPackExternalReviewRequestBoundaryContract(input) {
  ...
  if (computed.decision !== 'plan_pack_external_review_request_boundary_prepared') {
    return {
      ...failure('plan_pack_external_review_request_boundary_not_ready'),
      decision: computed.decision,
      blockers: computed.blockers
    };
  }
  ...
  return {
    ...
    requestedReviewEvidenceFields: REQUIRED_EXTERNAL_REVIEW_ROUTE_KEYS,
    futureReviewRequests: futureReviewRequestEntries(),
    routeSourceTaskId: input.routeSource.sourceTaskId,
    routeSourceValidationId: input.routeSource.sourceValidationId,
    nextGate: 'await_observation_external_review_and_tag_approval_before_completion_or_release_actions',
    reviewAcceptedByThisContract: false,
    tagApprovalAcceptedByThisContract: false,
    reviewAppliedByThisContract: false,
    completionAuditPatchApplied: false,
...
module.exports = {
  ...
  PHASE10_REQUIREMENT_ID,
  PHASE9_REQUIREMENT_ID,
