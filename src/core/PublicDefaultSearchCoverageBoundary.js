...
const TASK_ID = 'CM-1018_PUBLIC_DEFAULT_SEARCH_COVERAGE_BOUNDARY';
const STATUS_ACCEPTED = 'PUBLIC_DEFAULT_SEARCH_COVERAGE_ACCEPTED_NOT_READY';
const STATUS_BLOCKED = 'PUBLIC_DEFAULT_SEARCH_COVERAGE_BLOCKED_NOT_READY';

const ALLOWED_DECISIONS = new Set([
  'PUBLIC_DEFAULT_SEARCH_COVERAGE_PASSED_NOT_READY'
]);
...
const ZERO_REQUIRED_COUNTER_KEYS = Object.freeze([
  'recordMemoryCalls',
  'providerCalls',
  'apiCalls',
  'publicMcpExpansion',
...
function collectCounterBlockers(counters, rawCounters, markerCount) {
  const blockers = [];
  ...
  if (isNonNegativeInteger(counters.searchMemoryCalls) && counters.searchMemoryCalls !== markerCount) {
    blockers.push('counter_searchMemoryCalls_must_match_marker_count');
  }
  if (isNonNegativeInteger(counters.syncCalls) && counters.syncCalls !== markerCount) {
    blockers.push('counter_syncCalls_must_match_marker_count');
  }
  if (isNonNegativeInteger(counters.rawDurableMemoryReads) && counters.rawDurableMemoryReads !== markerCount) {
    blockers.push('counter_rawDurableMemoryReads_must_match_marker_count');
  }
  if (
    isNonNegativeInteger(counters.durableRecallAuditWrites)
    && counters.durableRecallAuditWrites !== markerCount
  ) {
    blockers.push('counter_durableRecallAuditWrites_must_match_marker_count');
  }

  for (const key of ZERO_REQUIRED_COUNTER_KEYS) {
    if (isNonNegativeInteger(counters[key]) && counters[key] !== 0) {
      blockers.push(`counter_${key}_must_be_zero`);
    }
  }

  const boundedCounterLimits = {
    candidateCacheWrites: markerCount,
    candidateCacheFlushes: markerCount * 3,
    vectorFlushes: markerCount * 6,
    embeddingCacheWrites: markerCount * 5
  };

  for (const [key, limit] of Object.entries(boundedCounterLimits)) {
    if (isNonNegativeInteger(counters[key]) && counters[key] > limit) {
      blockers.push(`counter_${key}_exceeds_public_default_search_budget`);
    }
  }
...
function collectClaimBlockers(claims) {
  ...
  if (safeClaims.publicSearchReliableClaimed !== false) {
    blockers.push('claim_publicSearchReliableClaimed_must_be_false');
  }
...
function normalizeMarkerEvidence(value = {}) {
  ...
  return {
    ...
    expectedMemoryIdHashesOrOpaqueIds: normalizeStringArray(
      safeValue.expectedMemoryIdHashesOrOpaqueIds
    ),
    ...
    rawOutputPrinted: safeValue.rawOutputPrinted
  };
}
...
function collectMarkerBlockers(marker, index) {
  ...
  if (marker.expectedMemoryIdHashesOrOpaqueIds.length === 0) {
    blockers.push(`${prefix}_expected_id_hashes_missing`);
  }
  ...
  if (marker.rawOutputPrinted !== false) {
    blockers.push(`${prefix}_raw_output_printed_must_be_false`);
  }

  const resultIds = new Set(marker.resultIdHashesOrStableOpaqueIds);
  const expectedIds = marker.expectedMemoryIdHashesOrOpaqueIds;
...
function evaluatePublicDefaultSearchCoverageBoundary(input = {}) {
  ...
  if (!proofRunId) blockers.push('proofRunId_missing');
  ...
  if (safeInput.publicDefaultSearch !== true) blockers.push('publicDefaultSearch_must_be_true');
  if (safeInput.internalNoRawAdapterUsed !== false) {
    blockers.push('internalNoRawAdapterUsed_must_be_false');
  }
  if (safeInput.requestContextNoTokenReadOnly !== false) {
    blockers.push('requestContextNoTokenReadOnly_must_be_false');
  }
  if (safeInput.rawOutputPrinted !== false) blockers.push('rawOutputPrinted_must_be_false');
  ...
  return {
    ...
    safety: {
      ...
      allowsLocalDefaultSearchSideEffects: true,
      claimsWriteReliable: false,
      claimsRecallReliable: false,
      claimsPublicSearchReliable: false,
