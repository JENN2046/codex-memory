...
const CONTRACT_NAME = 'VcpMemoryTrustedWriteProposalExactBoundaryPacketSkeletonContract';
const CONTRACT_MODE = 'fixture_trusted_write_proposal_exact_boundary_packet_skeleton_only';
const SCHEMA_VERSION = 1;

const PACKET_CONTRACT_VERSION = 'vcp_memory_trusted_write_proposal_exact_boundary_packet_skeleton_v1';
const PACKET_ID_PREFIX = 'm9_fixture_trusted_write_proposal_exact_boundary_packet_';

const ALLOWED_DECISIONS = Object.freeze([
  'packet_skeleton_accept_non_authorizing',
  'packet_skeleton_incomplete',
  'stop_l4'
]);
...
const ALLOWED_DISCLOSURE_LEVELS = Object.freeze([
  'redacted_packet_skeleton',
  'field_names_only',
  'none'
]);

const ALLOWED_NEXT_ACTIONS = Object.freeze([
  'cm1826_fixture_contract',
...
const REQUIRED_EVIDENCE_FIELDS = Object.freeze([
  ...
  'cm1823_local_fixture_preparation_closeout_present',
  'cm1824_executable_closeout_gate_fixture_present',
  'local_fixture_contract_preparation_slice_closed'
]);

const REQUIRED_BOUNDARY_FIELDS = Object.freeze([
  'target_alias_candidate_present',
  'transport_candidate_present',
  'exact_target_bound',
  ...
  'exact_proposal_operations_bound',
...
function safePacketId(input) {
  const packetId = isPlainObject(input?.packet) ? input.packet.packet_id : null;
  return isFixtureIdentifier(packetId, PACKET_ID_PREFIX) ? packetId : null;
}
...
function lowDisclosureProjection(input) {
  ...
  return {
    ...
    packetId: safePacketId(input),
    acceptedM8TrustedFullReadWorkflowPresentForPlanning: isPlainObject(evidence)
      ? evidence.accepted_m8_trusted_full_read_workflow_present_for_planning === true
      : false,
    cm1821EnvelopeFixtureContractPresent: isPlainObject(evidence)
      ? evidence.cm1821_envelope_fixture_contract_present === true
      : false,
    cm1822ReceiptShapeFixtureContractPresent: isPlainObject(evidence)
      ? evidence.cm1822_receipt_shape_fixture_contract_present === true
      : false,
    cm1824ExecutableCloseoutGateFixturePresent: isPlainObject(evidence)
      ? evidence.cm1824_executable_closeout_gate_fixture_present === true
      : false
  };
}
...
function sideEffectPosture(computedDecision = null) {
  return {
    exactBoundaryPacketSkeletonAccepted:
      computedDecision === 'packet_skeleton_accept_non_authorizing',
    nonAuthorizingPacketSkeletonOnly: true,
...
function validateShape(input) {
  ...
  if (!isFixtureIdentifier(input.packet.packet_id, PACKET_ID_PREFIX)) invalid.push('packet.packet_id');
  if (input.packet.contract_version !== PACKET_CONTRACT_VERSION) invalid.push('packet.contract_version');
  ...
  if (input.packet.packet_skeleton_only !== true) invalid.push('packet.packet_skeleton_only');
...
function hasL4Stop(input) {
  ...
  return (
    ...
    boundary.exact_proposal_operations_bound ||
...
function isSkeletonBoundaryDeclared(boundary) {
  return (
    boundary.target_alias_candidate_present &&
    boundary.transport_candidate_present &&
    boundary.missing_exact_fields_declared &&
    boundary.exact_boundary_complete === false
  );
}

function computeDecision(input) {
  if (hasL4Stop(input)) return 'stop_l4';
  if (!isPreparationEvidenceComplete(input.evidence)) return 'packet_skeleton_incomplete';
  if (!isSkeletonBoundaryDeclared(input.boundary)) return 'packet_skeleton_incomplete';
  return 'packet_skeleton_accept_non_authorizing';
}

function validateVcpMemoryTrustedWriteProposalExactBoundaryPacketSkeletonContract(input) {
  ...
  if (invalidFields.length > 0) {
    return rejected('invalid_trusted_write_proposal_exact_boundary_packet_skeleton_contract', input, {
      invalidFields
    });
  }
  ...
  return {
    ...
    forbiddenCounters: [],
    invalidFields: [],
    nextAction: 'packet_skeleton_validated_no_authority',
