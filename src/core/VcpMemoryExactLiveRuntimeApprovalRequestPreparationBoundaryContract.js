...
const CONTRACT_NAME = 'VcpMemoryExactLiveRuntimeApprovalRequestPreparationBoundaryContract';
const CONTRACT_MODE = 'fixture_exact_live_runtime_approval_request_preparation_boundary_only';
const SCHEMA_VERSION = 1;
const CONTRACT_VERSION = 'vcp_memory_exact_live_runtime_approval_request_preparation_boundary_v1';
const BOUNDARY_ID_PREFIX = 'cm1881_fixture_exact_live_runtime_approval_request_preparation_boundary_';

const ALLOWED_EVIDENCE_TYPES = Object.freeze(['fixture-only', 'schema-only']);
const ALLOWED_PROFILES = Object.freeze(['exact-live-runtime-approval-request-preparation-boundary']);
...
const REQUIRED_SOURCE_FIELDS = Object.freeze([
  'cm1877_assembly_boundary_review_present',
  'cm1878_assembly_boundary_fixture_contract_present',
  'cm1879_assembly_boundary_fixture_closeout_present',
  'cm1880_preparation_boundary_review_present',
  'validation_cmv1983_present'
]);

const REQUIRED_BOUNDARY_FIELDS = Object.freeze([
  'preparation_phase_names_declared',
  'required_evidence_categories_declared',
  'missing_value_classifications_declared',
  'required_authority_categories_declared',
  'disclosure_policy_categories_declared',
  'abort_condition_categories_declared',
  'validation_evidence_references_declared',
  'false_zero_counter_policy_declared',
  'category_only_boundary_allowed',
  'executable_request_preparation_allowed',
  'request_packet_creation_allowed',
  'concrete_values_allowed',
...
function lowDisclosureProjection(input) {
  ...
  return {
    ...
    cm1880PreparationBoundaryReviewPresent: isPlainObject(sources)
      ? sources.cm1880_preparation_boundary_review_present === true
      : false,
    validationCmv1983Present: isPlainObject(sources)
      ? sources.validation_cmv1983_present === true
      : false,
    categoryOnlyBoundaryAllowed: isPlainObject(boundary)
      ? boundary.category_only_boundary_allowed === true
      : false,
    executableRequestPreparationAllowed: isPlainObject(boundary)
      ? boundary.executable_request_preparation_allowed === true
      : false,
    requestPacketCreationAllowed: isPlainObject(boundary)
      ? boundary.request_packet_creation_allowed === true
      : false,
    concreteValuesAllowed: isPlainObject(boundary)
      ? boundary.concrete_values_allowed === true
      : false,
    requestAssemblyAllowed: isPlainObject(boundary)
      ? boundary.request_assembly_allowed === true
      : false
  };
}
...
function sideEffectPosture(computedDecision = null) {
  return {
    preparationBoundaryAccepted:
      computedDecision === 'preparation_boundary_accepted_category_only_non_authorizing',
    nonAuthorizingCategoryBoundaryOnly: true,
    executableRequestPreparationAllowed: false,
    requestPacketCreationAllowed: false,
    concreteValuesAllowed: false,
...
function hasL4Stop(input) {
  ...
  return (
    boundary.executable_request_preparation_allowed ||
    boundary.request_packet_creation_allowed ||
    boundary.concrete_values_allowed ||
...
function isBoundaryComplete(boundary) {
  return (
    boundary.preparation_phase_names_declared === true &&
    boundary.required_evidence_categories_declared === true &&
    boundary.missing_value_classifications_declared === true &&
    boundary.required_authority_categories_declared === true &&
    boundary.disclosure_policy_categories_declared === true &&
    ...
    boundary.category_only_boundary_allowed === true &&
    boundary.executable_request_preparation_allowed === false &&
    boundary.request_packet_creation_allowed === false &&
    boundary.concrete_values_allowed === false &&
...
function computeDecision(input) {
  ...
  if (!isSourceChainComplete(input.sources)) return 'preparation_boundary_incomplete';
  if (!isBoundaryComplete(input.boundary)) return 'preparation_boundary_incomplete';
  return 'preparation_boundary_accepted_category_only_non_authorizing';
}

function validateVcpMemoryExactLiveRuntimeApprovalRequestPreparationBoundaryContract(input) {
  ...
  if (invalidFields.length > 0) {
    return rejected('invalid_exact_live_runtime_approval_request_preparation_boundary_contract', input, {
      invalidFields
    });
  }
  ...
  return {
    ...
    forbiddenCounters: [],
    invalidFields: [],
    nextAction: 'preparation_boundary_validated_category_only_no_authority',
