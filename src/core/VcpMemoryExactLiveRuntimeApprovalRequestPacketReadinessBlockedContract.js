...
const CONTRACT_NAME =
  'VcpMemoryExactLiveRuntimeApprovalRequestPacketReadinessBlockedContract';
const CONTRACT_MODE =
...
const READINESS_ID_PREFIX =
  'cm1893_fixture_exact_live_runtime_approval_request_packet_readiness_blocked_';

const ALLOWED_EVIDENCE_TYPES = Object.freeze(['fixture-only', 'schema-only']);
const ALLOWED_PROFILES = Object.freeze([
  'exact-live-runtime-approval-request-packet-readiness-blocked'
]);
...
const ALLOWED_DISCLOSURE_LEVELS = Object.freeze([
  'request_packet_readiness_blocked_categories_only',
  'none'
]);
...
const REQUIRED_PACKET_FIELDS = Object.freeze([
  ...
  'profile',
  'non_authorizing',
  'request_packet_readiness_blocked_fixture_only'
]);

const REQUIRED_EVIDENCE_FIELDS = Object.freeze([
  'cm1889_skeleton_review_boundary_present',
  'cm1890_skeleton_boundary_fixture_contract_present',
  'cm1891_skeleton_fixture_closeout_present',
  'cm1892_readiness_blocked_precondition_refresh_present',
  'validation_cmv1995_present'
]);

const REQUIRED_BLOCKED_READINESS_FIELDS = Object.freeze([
  'request_packet_readiness_precondition_reviewed',
  'skeleton_evidence_is_request_packet_readiness',
  'request_packet_readiness_may_open',
  'request_packet_readiness_blocked',
  'request_packet_readiness_block_reason_declared',
...
function lowDisclosureProjection(input) {
  ...
  return {
    ...
    cm1892ReadinessBlockedPreconditionRefreshPresent: isPlainObject(evidence)
      ? evidence.cm1892_readiness_blocked_precondition_refresh_present === true
      : false,
    requestPacketReadinessBlocked: isPlainObject(blockedReadiness)
      ? blockedReadiness.request_packet_readiness_blocked === true
      : false,
    requestPacketReadinessMayOpen: isPlainObject(blockedReadiness)
      ? blockedReadiness.request_packet_readiness_may_open === true
      : false,
...
function sideEffectPosture(computedDecision = null) {
  return {
    requestPacketReadinessBlockedFixtureAccepted:
      computedDecision === 'request_packet_readiness_blocked_missing_exact_request_packet_material',
    requestPacketReadinessMayOpen: false,
    requestPacketReadinessBlocked: true,
    skeletonEvidenceIsRequestPacketReadiness: false,
...
function validateShape(input) {
  ...
  if (input.packet.request_packet_readiness_blocked_fixture_only !== true) {
    invalid.push('packet.request_packet_readiness_blocked_fixture_only');
  }
...
function hasL4Stop(input) {
  ...
  return (
    readiness.skeleton_evidence_is_request_packet_readiness ||
    ...
    readiness.approval_grant_present ||
...
function isBlockedReadinessComplete(blockedReadiness) {
  return (
    blockedReadiness.request_packet_readiness_precondition_reviewed === true &&
    blockedReadiness.request_packet_readiness_blocked === true &&
    blockedReadiness.request_packet_readiness_block_reason_declared === true &&
...
function computeDecision(input) {
  ...
  if (!isEvidenceComplete(input.evidence)) return 'request_packet_readiness_incomplete';
  if (!isBlockedReadinessComplete(input.blockedReadiness)) {
    return 'request_packet_readiness_incomplete';
  }
  return 'request_packet_readiness_blocked_missing_exact_request_packet_material';
}

function validateVcpMemoryExactLiveRuntimeApprovalRequestPacketReadinessBlockedContract(input) {
  ...
  if (forbiddenFields.length > 0) {
    return rejected('forbidden_raw_secret_exact_value_packet_request_or_overclaim_fields', input, {
      forbiddenFields
    });
  }
  ...
  return {
    ...
    forbiddenCounters: [],
    invalidFields: [],
    nextAction: 'request_packet_readiness_blocked_validated_no_authority',
