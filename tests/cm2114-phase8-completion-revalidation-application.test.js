...
test('CM-2114 fails closed on proof, bundle, replay, baseline, side effect, or readiness drift', () => {
  for (const mutate of [
    ...
    value => { value.decision.applicationAuthorization.productionReady = true; },
    value => { value.baseline.phase8CompletionStatus = 'complete'; },
    value => { value.decision.applicationSideEffectLimits.nativeWrites = 1; },
    value => { value.decision.applicationSideEffectLimits.productionReady = true; },
    ...
    value => { value.decision.nonClaims.unreviewedAuthorityAccepted = false; },
...
test('CM-2114 application receipt rejects replay, new runtime action, or readiness overclaim', () => {
  ...
  for (const mutate of [
    ...
    payload => { payload.phaseAudit.phaseId = 'phase7_forged'; },
    payload => { delete payload.phaseAudit.phaseId; },
    ...
    payload => { payload.applicationCounters.providerCalls = 1; },
