...
} = require('../src/core/VcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract');

function zeroCounters(overrides = {}) {
  return Object.fromEntries(ZERO_COUNTER_FIELDS.map(field => [field, overrides[field] ?? 0]));
}

function authorizationRequestReadinessBlocked(overrides = {}) {
  const packet = {
    readiness_id:
      'cm1902_fixture_exact_live_runtime_authorization_request_readiness_blocked_001',
    contract_version:
      'vcp_memory_exact_live_runtime_authorization_request_readiness_blocked_v1',
    evidence_type: 'fixture-only',
    profile: 'exact-live-runtime-authorization-request-readiness-blocked',
    non_authorizing: true,
    authorization_request_readiness_blocked_fixture_only: true
  };

  const evidence = Object.fromEntries(REQUIRED_EVIDENCE_FIELDS.map(field => [field, true]));

  const blockedReadiness = {
    authorization_request_readiness_precondition_reviewed: true,
    ...
    authorization_request_readiness_block_reason_declared: true,
  ...
  const output = {
    disclosure_level: 'authorization_request_readiness_blocked_categories_only',
  ...
  return {
    ...
    nextActionAllowed:
      overrides.nextActionAllowed || 'cm1902_authorization_request_readiness_blocked_fixture_contract',
...
test('CM1902 accepts authorization request readiness as blocked fixture only', () => {
  const result = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked()
  );
  ...
  assert.equal(result.authorizationRequestReadinessBlockedFixtureAccepted, true);
  ...
  assert.equal(result.boundaryEvidenceIsAuthorizationRequestReadiness, false);
  assert.equal(result.fixtureCloseoutIsAuthorizationRequestReadiness, false);
...
test('CM1902 reports incomplete when evidence or blocked-readiness declaration is absent', () => {
  const result = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      evidence: {
        cm1901_authorization_request_readiness_blocked_precondition_refresh_present: false,
        validation_cmv2004_present: false
      },
      blockedReadiness: {
        authorization_request_readiness_precondition_reviewed: false,
        authorization_request_readiness_block_reason_declared: false,
        missing_authorities_declared: false,
        next_fixture_contract_allowed: false
      },
      expectedDecision: 'authorization_request_readiness_incomplete'
    })
  );

  assert.equal(result.accepted, true);
  assert.equal(result.decision, 'authorization_request_readiness_incomplete');
  assert.equal(result.authorizationRequestReadinessBlockedFixtureAccepted, false);
  assert.equal(result.authorizationRequestReadinessBlocked, true);
  assert.equal(result.authorizationRequestReady, false);
  assert.equal(result.requestPacketCreated, false);
  assert.equal(result.memoryWritten, false);
...
test('CM1902 stops authorization request packet runtime memory config and readiness as L4', () => {
  const result = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
...
test('CM1902 rejects raw secret authorization request approval runtime memory config fields without echo', () => {
  const result = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      packet: {
...
test('CM1902 rejects unexpected fields without echoing submitted values', () => {
  const result = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
...
test('CM1902 rejects missing positive and malformed zero side-effect counters', () => {
  const missingFixture = authorizationRequestReadinessBlocked();
  delete missingFixture.counters.providerApiCalls;

  const missingResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    missingFixture
  );
  ...
  const positiveResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      counters: {
  ...
  const malformedResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      counters: {
        providerApiCalls: '0'
      }
...
test('CM1902 rejects invalid packet fields decision mismatch and unsafe decision echo', () => {
  const invalidResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      packet: {
        readiness_id: 'not-a-valid-readiness-id',
        contract_version: 'wrong',
        authorization_request_readiness_blocked_fixture_only: false
      },
      output: {
  ...
  assert.ok(
    invalidResult.invalidFields.includes('packet.authorization_request_readiness_blocked_fixture_only')
  );
  assert.ok(invalidResult.invalidFields.includes('output.disclosure_level'));

  const mismatchResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
      blockedReadiness: {
        authorization_request_readiness_precondition_reviewed: false
      }
  ...
  assert.equal(mismatchResult.computedDecision, 'authorization_request_readiness_incomplete');

  const unsafeDecisionResult = validateVcpMemoryExactLiveRuntimeAuthorizationRequestReadinessBlockedContract(
    authorizationRequestReadinessBlocked({
...
test('CM1902 locks authorization request readiness blocked vocabulary and no-side-effect posture', () => {
