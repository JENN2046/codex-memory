...
test('CM1448 release test gate matrix matches default-safe excluded categories', () => {
  ...
  assert.equal(
    ...
    'node --test --test-concurrency=1 tests/cm2118-full-plan-application-execution.test.js tests/cm2122-full-plan-status-sync-execution.test.js tests/cm2126-exact-branch-cas-execution.test.js'
  );
});
