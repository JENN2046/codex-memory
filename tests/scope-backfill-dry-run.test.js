...
test('scope-backfill-dry-run CLI should include suggestedDefaults', () => {
  ...
  assert.ok(report.suggestedDefaults.workspace_id);
});

test('scope-backfill-dry-run CLI should count records that only miss workspace_id', () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'backfill-workspace-only-'));
  try {
    seedMemoryRecords(path.join(tmpDir, 'data', 'codex-memory.sqlite'), [
      {
        memoryId: 'missing-workspace-only',
        title: 'Missing Workspace Only',
    ...
    assert.equal(report.alreadyScoped, 0);
    assert.equal(report.missingProjectId, 0);
    assert.equal(report.missingClientId, 0);
    assert.equal(report.missingWorkspaceId, 1);
    assert.equal(report.missingVisibility, 0);
