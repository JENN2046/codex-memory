...
test('runtime proof collector waits for explicit inputs without claiming readiness', () => {
  ...
  assert.equal(report.summary.availableUnitCount, 15);
  assert.equal(report.summary.executedUnitCount, 0);
  assert.equal(report.summary.acceptedUnitCount, 0);
  assert.equal(report.summary.missingUnitCount, 15);
  ...
  assert.equal(
    report.units.httpRuntimeObservabilityOperationProof.status,
    'not_supplied'
  );
  assert.equal(
    report.units.evidenceRuntimeTraceProof.status,
    'not_supplied'
  );
  assert.equal(
    report.units.evidenceManifestProof.status,
    'not_supplied'
  );
  assert.equal(
    report.units.a5RuntimeAuthorizationPreconditionProof.status,
    'not_supplied'
  );
...
test('runtime proof collector executes source registry proof from explicit sanitized input only', () => {
  ...
  assert.equal(report.summary.missingUnitCount, 14);
...
test('runtime proof collector executes http runtime observability operation proof from explicit sanitized input only', () => {
  const report = collectValidationAggregatorRuntimeProofUnits({
    httpRuntimeObservabilityOperationProof:
      buildHttpRuntimeObservabilityOperationProofInput()
  });
  ...
  assert.equal(report.summary.governanceRuntimeLoopGapProofAccepted, false);
  assert.equal(report.summary.recallIsolationRuntimeProofAccepted, false);
  assert.equal(report.summary.migrationImportExportBackupRestoreApprovalProofAccepted, false);
  assert.equal(report.summary.httpRuntimeObservabilityOperationProofAccepted, true);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.executed, true);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.accepted, true);
  assert.equal(
    report.units.httpRuntimeObservabilityOperationProof.status,
    'boundary_accepted_http_operation_still_blocked'
  );
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.sourceEvidence.exact, true);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.observabilitySurfaces.exact, true);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.runtimeEvidence.requiredExact, true);
  assert.deepEqual(report.units.httpRuntimeObservabilityOperationProof.failClosedReasons, []);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.readiness.httpRuntimeObserved, false);
  assert.equal(report.canClaimRuntimeReady, false);
  assert.equal(report.canClaimFinalRcReady, false);
  assert.equal(report.canClaimV1RcReady, false);
});

test('runtime proof collector executes evidence runtime trace proof from explicit sanitized input only', () => {
  const report = collectValidationAggregatorRuntimeProofUnits({
    evidenceRuntimeTraceProof:
      buildEvidenceRuntimeTraceProofInput()
  });
  ...
  assert.equal(report.summary.recallIsolationRuntimeProofAccepted, false);
  assert.equal(report.summary.migrationImportExportBackupRestoreApprovalProofAccepted, false);
  assert.equal(report.summary.httpRuntimeObservabilityOperationProofAccepted, false);
  assert.equal(report.summary.evidenceRuntimeTraceProofAccepted, true);
  assert.equal(report.units.evidenceRuntimeTraceProof.executed, true);
  assert.equal(report.units.evidenceRuntimeTraceProof.accepted, true);
  assert.equal(
    report.units.evidenceRuntimeTraceProof.status,
    'trace_accepted_runtime_still_blocked'
  );
  assert.equal(report.units.evidenceRuntimeTraceProof.sourceEvidence.exact, true);
  assert.equal(report.units.evidenceRuntimeTraceProof.runtimeGaps.exact, true);
  assert.equal(report.units.evidenceRuntimeTraceProof.traceLinks.safe, true);
  assert.deepEqual(report.units.evidenceRuntimeTraceProof.failClosedReasons, []);
  assert.equal(report.units.evidenceRuntimeTraceProof.readiness.runtimeEnforcementReady, false);
  assert.equal(report.canClaimRuntimeReady, false);
  assert.equal(report.canClaimFinalRcReady, false);
  assert.equal(report.canClaimV1RcReady, false);
});

test('runtime proof collector executes evidence manifest proof from explicit sanitized input only', () => {
  const report = collectValidationAggregatorRuntimeProofUnits({
    evidenceManifestProof:
      buildEvidenceManifestProofInput()
  });
  ...
  assert.equal(report.summary.migrationImportExportBackupRestoreApprovalProofAccepted, false);
  assert.equal(report.summary.httpRuntimeObservabilityOperationProofAccepted, false);
  assert.equal(report.summary.evidenceRuntimeTraceProofAccepted, false);
  assert.equal(report.summary.evidenceManifestProofAccepted, true);
  assert.equal(report.units.evidenceManifestProof.executed, true);
  assert.equal(report.units.evidenceManifestProof.accepted, true);
  assert.equal(
    report.units.evidenceManifestProof.status,
    'evidence_manifest_accepted_runtime_still_blocked'
  );
  assert.equal(report.units.evidenceManifestProof.sourceContract.safe, true);
  assert.equal(report.units.evidenceManifestProof.sourceEvidence.exact, true);
  assert.equal(report.units.evidenceManifestProof.criticalGateSemantics.failureStatesFailClosed, true);
  assert.equal(report.units.evidenceManifestProof.blockedActions.exact, true);
  assert.deepEqual(report.units.evidenceManifestProof.failClosedReasons, []);
  assert.equal(report.units.evidenceManifestProof.readiness.runtimeReady, false);
  assert.equal(report.canClaimRuntimeReady, false);
  assert.equal(report.canClaimFinalRcReady, false);
  assert.equal(report.canClaimV1RcReady, false);
});

test('runtime proof collector executes A5 runtime authorization precondition proof from explicit sanitized input only', () => {
  const report = collectValidationAggregatorRuntimeProofUnits({
    a5RuntimeAuthorizationPreconditionProof:
      buildA5RuntimeAuthorizationPreconditionProofInput()
  });
  ...
  assert.equal(report.summary.httpRuntimeObservabilityOperationProofAccepted, false);
  assert.equal(report.summary.evidenceRuntimeTraceProofAccepted, false);
  assert.equal(report.summary.evidenceManifestProofAccepted, false);
  assert.equal(report.summary.a5RuntimeAuthorizationPreconditionProofAccepted, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.executed, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.accepted, true);
  assert.equal(
    report.units.a5RuntimeAuthorizationPreconditionProof.status,
    'a5_authorization_precondition_accepted_runtime_still_blocked'
  );
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.publicMcpTools.exact, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.preAuthorizationEvidence.exact, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.authorizationActions.exact, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.bundledApprovals.exact, true);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.forbiddenClaims.exact, true);
  assert.deepEqual(report.units.a5RuntimeAuthorizationPreconditionProof.failClosedReasons, []);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.readiness.authorizationGranted, false);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.readiness.runtimeReady, false);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.canClaimRuntimeReady, false);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.canClaimFinalRcReady, false);
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.canClaimV1RcReady, false);
  assert.equal(report.canClaimRuntimeReady, false);
  assert.equal(report.canClaimFinalRcReady, false);
  assert.equal(report.canClaimV1RcReady, false);
});

test('runtime proof collector aggregates accepted explicit source registry freshness baseline summary missing-stale unsupported-source no-touch overclaim governance recall isolation migration approval http observability evidence trace evidence manifest and A5 authorization precondition units', () => {
  const report = collectValidationAggregatorRuntimeProofUnits({
    sourceRegistryProof: buildSourceRegistryProofInput(),
    evidenceFreshnessProof: buildEvidenceFreshnessProofInput(),
    baselineBindingProof: buildBaselineBindingProofInput(),
  ...
  assert.equal(report.summary.availableUnitCount, 15);
  assert.equal(report.summary.executedUnitCount, 15);
  assert.equal(report.summary.acceptedUnitCount, 15);
  assert.equal(report.summary.rejectedUnitCount, 0);
...
test('runtime proof collector fails closed when A5 authorization action is granted', () => {
  const authorizationInput = buildA5RuntimeAuthorizationPreconditionProofInput();
  const report = collectValidationAggregatorRuntimeProofUnits({
    a5RuntimeAuthorizationPreconditionProof:
      buildA5RuntimeAuthorizationPreconditionProofInput({
        authorizationActions: authorizationInput.authorizationActions.map((action, index) =>
          index === 0
            ? { ...action, granted: true }
            : action
        )
  ...
  assert.equal(report.units.a5RuntimeAuthorizationPreconditionProof.accepted, false);
  assert.equal(
    report.units.a5RuntimeAuthorizationPreconditionProof.failClosedReasons.includes(
      'authorization_action_granted_or_not_separate_a5'
    ),
    true
  );
  assert.equal(
    report.units.a5RuntimeAuthorizationPreconditionProof.readiness.authorizationGranted,
    false
  );
  assert.equal(report.summary.validationAggregatorFullImplementation, false);
  assert.equal(report.summary.v1RcReady, false);
...
test('runtime proof collector fails closed when http observability surface claims execution', () => {
  const httpObservabilityInput = buildHttpRuntimeObservabilityOperationProofInput();
  const report = collectValidationAggregatorRuntimeProofUnits({
    httpRuntimeObservabilityOperationProof:
      buildHttpRuntimeObservabilityOperationProofInput({
        observabilitySurfaces: httpObservabilityInput.observabilitySurfaces.map(item =>
          item.id === 'http_observe_cli'
            ? { ...item, executedInThisPhase: true }
            : item
        )
      })
  });
  ...
  assert.equal(report.summary.acceptedUnitCount, 0);
  assert.equal(report.summary.rejectedUnitCount, 1);
  assert.equal(report.units.httpRuntimeObservabilityOperationProof.accepted, false);
  assert.equal(
    report.units.httpRuntimeObservabilityOperationProof.failClosedReasons.includes(
      'surface_claims_execution_or_readiness'
    ),
    true
  );
  assert.deepEqual(
    report.units.httpRuntimeObservabilityOperationProof.observabilitySurfaces.unsafeIds,
    ['http_observe_cli']
  );
  assert.equal(report.summary.validationAggregatorFullImplementation, false);
  assert.equal(report.summary.v1RcReady, false);
...
test('runtime proof collector fails closed when evidence runtime trace source claims authority', () => {
  const traceInput = buildEvidenceRuntimeTraceProofInput();
  const report = collectValidationAggregatorRuntimeProofUnits({
    evidenceRuntimeTraceProof:
      buildEvidenceRuntimeTraceProofInput({
        sourceEvidence: traceInput.sourceEvidence.map(item =>
          item.id === 'p52_schema_version_boundary_contract'
            ? { ...item, runtimeAuthority: true }
            : item
        )
      })
  });
  ...
  assert.equal(report.summary.acceptedUnitCount, 0);
  assert.equal(report.summary.rejectedUnitCount, 1);
  assert.equal(report.units.evidenceRuntimeTraceProof.accepted, false);
  assert.equal(
    report.units.evidenceRuntimeTraceProof.failClosedReasons.includes(
      'source_claims_runtime_authority'
    ),
    true
  );
  assert.deepEqual(
    report.units.evidenceRuntimeTraceProof.sourceEvidence.unsafeIds,
...
test('runtime proof collector fails closed when evidence manifest accepts public MCP expansion', () => {
  const manifestInput = buildEvidenceManifestProofInput();
  const report = collectValidationAggregatorRuntimeProofUnits({
    evidenceManifestProof:
      buildEvidenceManifestProofInput({
        acceptedSourceTypes: [
          ...manifestInput.acceptedSourceTypes,
  ...
  assert.equal(report.summary.acceptedUnitCount, 0);
  assert.equal(report.summary.rejectedUnitCount, 1);
  assert.equal(report.units.evidenceManifestProof.accepted, false);
  assert.equal(
    report.units.evidenceManifestProof.failClosedReasons.includes(
      'unsupported_source_type_or_contract_drift'
    ),
    true
  );
  assert.deepEqual(
    report.units.evidenceManifestProof.sourceContract.unsupportedSourceTypes,
    ['public_mcp_expansion']
  );
  assert.equal(report.summary.validationAggregatorFullImplementation, false);
  assert.equal(report.summary.v1RcReady, false);
...
test('ValidationAggregator report surfaces runtime proof collector without readiness claim', () => {
  ...
  assert.equal(report.summary.validationAggregatorRuntimeProofCollectorAcceptedUnitCount, 15);
  assert.equal(report.summary.validationAggregatorRuntimeProofCollectorExecutedUnitCount, 15);
  ...
  assert.equal(
    report.evidence.p66ValidationAggregatorRuntimeProofCollector.units
      .httpRuntimeObservabilityOperationProof.accepted,
    true
  );
  assert.equal(
    report.evidence.p66ValidationAggregatorRuntimeProofCollector.units
      .evidenceRuntimeTraceProof.accepted,
    true
  );
  assert.equal(
    report.evidence.p66ValidationAggregatorRuntimeProofCollector.units
      .evidenceManifestProof.accepted,
    true
  );
  assert.equal(
    report.evidence.p66ValidationAggregatorRuntimeProofCollector.units
      .a5RuntimeAuthorizationPreconditionProof.accepted,
    true
  );
  assert.equal(report.summary.runtimeReady, false);
  assert.equal(report.summary.finalRcMatrixReady, false);
