...
const {
  ...
  ZERO_COUNTER_FIELDS,
  validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract
} = require('../src/core/VcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract');

function zeroCounters(overrides = {}) {
  return Object.fromEntries(ZERO_COUNTER_FIELDS.map(field => [field, overrides[field] ?? 0]));
}

function packetRefreshContract(overrides = {}) {
  const packet = {
    refresh_id: 'm9_fixture_trusted_write_proposal_exact_request_packet_refresh_blocked_001',
    contract_version: 'vcp_memory_trusted_write_proposal_exact_request_packet_refresh_blocked_v1',
    evidence_type: 'fixture-only',
    profile: 'trusted-write-proposal',
    non_authorizing: true,
    exact_request_packet_refresh_blocked_only: true
  };

  const evidence = Object.fromEntries(REQUIRED_EVIDENCE_FIELDS.map(field => [field, true]));

  const refresh = {
    local_packet_refresh_blocked_fixture_present: true,
  ...
  const output = {
    disclosure_level: 'redacted_packet_refresh',
  ...
  return {
    ...
    expectedDecision: overrides.expectedDecision || 'exact_request_packet_refresh_blocked_missing_exact_fields',
    nextActionAllowed: overrides.nextActionAllowed || 'cm1840_fixture_contract',
...
test('CM1840 accepts blocked exact request packet refresh without authority or runtime', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract()
  );

  assert.equal(result.accepted, true);
  assert.equal(result.contractMode, 'fixture_trusted_write_proposal_exact_request_packet_refresh_blocked_only');
  assert.equal(result.decision, 'exact_request_packet_refresh_blocked_missing_exact_fields');
  assert.equal(result.exactRequestPacketRefreshAccepted, true);
...
test('CM1840 reports exact_request_packet_refresh_incomplete when evidence or missing fields are absent', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      evidence: {
        cm1837_approval_request_readiness_blocked_contract_present: false,
        cm1839_blocked_preflight_present: false
      },
      refresh: {
        local_packet_refresh_blocked_fixture_present: false,
      ...
      expectedDecision: 'exact_request_packet_refresh_incomplete'
    })
  );

  assert.equal(result.accepted, true);
  assert.equal(result.decision, 'exact_request_packet_refresh_incomplete');
  assert.equal(result.exactRequestPacketRefreshAccepted, false);
  assert.equal(result.exactRequestPacketReady, false);
  assert.equal(result.approvalRequestReady, false);
  assert.equal(result.requestSubmitted, false);
  assert.equal(result.memoryWritten, false);
...
test('CM1840 stops packet request approval proposal runtime write and unlock claims as L4', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      refresh: Object.fromEntries(REQUIRED_REFRESH_FIELDS.map(field => [field, true])),
...
test('CM1840 rejects concrete raw secret request approval and readiness fields without echoing values', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      packet: {
      ...
      exactRequestPacketRefreshReady: 'SYNTHETIC_PACKET_REFRESH_READY_SHOULD_NOT_ECHO',
  ...
  assert.ok(result.forbiddenFields.includes('refresh.endpoint'));
  ...
  assert.ok(result.forbiddenFields.includes('exactRequestPacketRefreshReady'));
  ...
  assert.equal(serialized.includes('SYNTHETIC_PACKET_REFRESH_READY_SHOULD_NOT_ECHO'), false);
  assert.equal(serialized.includes('SYNTHETIC_RC_READY_SHOULD_NOT_ECHO'), false);
});

test('CM1840 rejects unexpected fields without echoing submitted values', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      extraRoot: 'SYNTHETIC_EXTRA_ROOT_SHOULD_NOT_ECHO',
      refresh: {
        extraRefresh: 'SYNTHETIC_EXTRA_REFRESH_SHOULD_NOT_ECHO'
      },
      missingExactFields: {
  ...
  assert.ok(result.unexpectedFields.includes('refresh.extraRefresh'));
  ...
  assert.equal(serialized.includes('SYNTHETIC_EXTRA_REFRESH_SHOULD_NOT_ECHO'), false);
  assert.equal(serialized.includes('SYNTHETIC_EXTRA_MISSING_SHOULD_NOT_ECHO'), false);
});

test('CM1840 rejects missing positive and malformed zero side-effect counters', () => {
  const missingFixture = packetRefreshContract();
  delete missingFixture.counters.providerApiCalls;

  const missingResult = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    missingFixture
  );
  ...
  const positiveResult = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      counters: {
  ...
  const malformedResult = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      counters: {
        providerApiCalls: '0'
      }
  ...
  assert.equal(
    ...
    'invalid_trusted_write_proposal_exact_request_packet_refresh_blocked_contract'
  );
  assert.ok(malformedResult.invalidFields.includes('counters.providerApiCalls'));
});

test('CM1840 rejects invalid packet fields decision mismatch and unsafe decision echo', () => {
  const unsafeExpectedDecision = 'PRIVATE_DECISION_SHOULD_NOT_ECHO';
  const invalid = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      packet: {
        refresh_id: 'unsafe_refresh_id',
  ...
  assert.equal(invalid.reasonCode, 'invalid_trusted_write_proposal_exact_request_packet_refresh_blocked_contract');
  assert.ok(invalid.invalidFields.includes('packet.refresh_id'));
  ...
  const mismatch = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract({
      refresh: {
        exact_request_packet_ready: true
      },
      expectedDecision: 'exact_request_packet_refresh_blocked_missing_exact_fields'
    })
  );
...
test('CM1840 locks packet-refresh vocabulary and no-side-effect posture', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPacketRefreshBlockedContract(
    packetRefreshContract()
  );

  assert.ok(ALLOWED_DECISIONS.includes('exact_request_packet_refresh_blocked_missing_exact_fields'));
  assert.ok(ALLOWED_DECISIONS.includes('exact_request_packet_refresh_incomplete'));
  assert.ok(ALLOWED_DECISIONS.includes('stop_l4'));
  assert.ok(ALLOWED_NEXT_ACTIONS.includes('cm1840_fixture_contract'));
  assert.ok(REQUIRED_EVIDENCE_FIELDS.includes('cm1839_blocked_preflight_present'));
  assert.ok(REQUIRED_REFRESH_FIELDS.includes('approval_request_body_prepared'));
  assert.ok(REQUIRED_MISSING_EXACT_FIELDS.includes('approval_request_body_missing'));
  ...
  assert.ok(FORBIDDEN_FIELD_NAMES.includes('exactRequestPacketRefreshReady'));
