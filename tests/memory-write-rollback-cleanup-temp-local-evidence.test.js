...
async function createTempLocalHarness() {
  const rootPath = await fs.mkdtemp(path.join(os.tmpdir(), 'codex-memory-cm1031-write-cleanup-'));
  const config = {
    defaultRequestSource: 'cm1031-write-cleanup-temp-local-evidence',
    ...
    embeddingFingerprint: 'cm1031-temp-local-write-cleanup-v1',
    embedDimensions: 32,
  ...
  const service = new MemoryWriteService({
    ...
    executionContextResolver: {
      resolve: () => ({
        agentAlias: 'Codex',
        agentId: 'cm1031-temp-local-fixture-agent',
        requestSource: 'cm1031-write-cleanup-temp-local-evidence'
      }),
...
function processPayload(overrides = {}) {
  return {
    target: 'process',
    title: 'Checkpoint: CM-1031 rollback cleanup temp-local evidence',
    content: [
      'Type: checkpoint',
      'CM1031 write rollback cleanup temp local marker',
      'Purpose: prove temp-local projection accounting and partial cleanup posture.',
      'Boundary: synthetic temp-local files only, no real memory, no provider, no readiness claim.'
    ].join('\n'),
    evidence: 'cm1031 synthetic temp-local rollback cleanup evidence',
    validated: true,
    reusable: false,
    tags: ['cm1031', 'write', 'rollback', 'cleanup', 'temp-local'],
    sensitivity: 'none',
    project_id: 'codex-memory',
    workspace_id: 'cm1031-write-cleanup-workspace',
    client_id: 'codex',
    task_id: 'CM-1031',
    conversation_id: 'cm1031-write-cleanup-temp-local',
    visibility: 'project',
...
test('CM-1031 accepted temp-local write exposes partial cleanup posture without hiding residuals', async () => {
  ...
  try {
    ...
    assert.equal(shadowBeforeCleanup.chunkCount >= 1, true);
    assert.equal(shadowBeforeCleanup.reconcileCount, 0);

    const vectorBeforeCleanup = await harness.vectorStore.getHealth();
    assert.equal(vectorBeforeCleanup.vectorCount, 1);
    assert.equal(vectorBeforeCleanup.embeddingCacheCount >= 1, true);
    ...
    await harness.candidateCacheStore.set('cm1031-cache-entry', {
    ...
    assert.equal(cacheBeforeCleanup.available, true);
    ...
    assert.equal(shadowAfterCleanup.reconcileCount, 0);
    ...
    assert.equal(vectorAfterCleanup.embeddingCacheCount >= 1, true);
