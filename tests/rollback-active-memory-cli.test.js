...
test('rollback active-memory CLI should run the repository standard suite in require-ready mode', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.ok, true);
    assert.equal(payload.summary.rollbackReady, true);
    assert.equal(payload.summary.totalCaseCount, 37);
    assert.equal(payload.summary.readyCaseCount, 37);
    assert.equal(payload.summary.notReadyCaseCount, 0);
    assert.equal(payload.summary.extendedMismatchCountTotal, 0);
    assert.deepEqual(payload.summary.recommendationBreakdown, { 'rollback-safe': 37 });
    assert.deepEqual(payload.summary.blockerBreakdown, {});
...
test('rollback active-memory CLI should filter standard suite cases by tag-all and exclude filters', async () => {
  ...
  try {
    ...
    assert.equal(excludeTagPayload.summary.totalCaseCount, 29);
    assert.equal(excludeTagPayload.summary.readyCaseCount, 29);
    ...
    assert.equal(excludeFixturePayload.summary.totalCaseCount, 36);
    assert.equal(excludeFixturePayload.summary.readyCaseCount, 36);
...
test('rollback active-memory CLI should filter standard suite cases by expectation and tool', async () => {
  ...
  try {
    ...
    assert.equal(expectationPayload.summary.totalCaseCount, 13);
    assert.equal(expectationPayload.summary.readyCaseCount, 13);
    ...
    assert.equal(expectationMultiPayload.summary.totalCaseCount, 37);
    assert.equal(expectationMultiPayload.summary.readyCaseCount, 37);
    ...
    assert.equal(expectationReversePayload.summary.totalCaseCount, 37);
    assert.equal(expectationReversePayload.summary.readyCaseCount, 37);
    ...
    assert.equal(expectationDupPayload.summary.totalCaseCount, 37);
    assert.equal(expectationDupPayload.summary.readyCaseCount, 37);
    ...
    assert.equal(toolPayload.summary.readyCaseCount, 16);
