...
test('rollback active-memory CLI should run the repository standard suite in require-ready mode', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.ok, true);
    assert.equal(payload.summary.rollbackReady, true);
    assert.equal(payload.summary.totalCaseCount, 43);
    assert.equal(payload.summary.readyCaseCount, 43);
    assert.equal(payload.summary.notReadyCaseCount, 0);
    assert.equal(payload.summary.extendedMismatchCountTotal, 0);
    assert.deepEqual(payload.summary.recommendationBreakdown, { 'rollback-safe': 43 });
    assert.deepEqual(payload.summary.blockerBreakdown, {});
...
test('rollback active-memory CLI should filter standard suite cases by multiple categories', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.readyCaseCount, 12);
    assert.equal(payload.summary.notReadyCaseCount, 0);
    assert.equal(payload.summary.ok, true);
    assert.equal(payload.summary.rollbackReady, true);
    ...
    assert.equal(payload.compareReport?.categoryAggregate.find(item => item.category === 'agent-selection')?.totalCaseCount, 8);
  } finally {
    await fs.rm(tempBasePath, { recursive: true, force: true });
  }
});
...
test('rollback active-memory CLI should filter standard suite cases by tag-all and exclude filters', async () => {
  ...
  try {
    ...
    assert.equal(excludeTagPayload.summary.totalCaseCount, 35);
    assert.equal(excludeTagPayload.summary.readyCaseCount, 35);
    ...
    assert.equal(excludeFixturePayload.summary.totalCaseCount, 42);
    assert.equal(excludeFixturePayload.summary.readyCaseCount, 42);
...
test('rollback active-memory CLI should filter standard suite cases by expectation and tool', async () => {
  ...
  try {
    ...
    assert.equal(expectationPayload.summary.totalCaseCount, 19);
    assert.equal(expectationPayload.summary.readyCaseCount, 19);
    ...
    assert.equal(expectationMultiPayload.summary.totalCaseCount, 43);
    assert.equal(expectationMultiPayload.summary.readyCaseCount, 43);
    ...
    assert.equal(expectationReversePayload.summary.totalCaseCount, 43);
    assert.equal(expectationReversePayload.summary.readyCaseCount, 43);
    ...
    assert.equal(expectationDupPayload.summary.totalCaseCount, 43);
    assert.equal(expectationDupPayload.summary.readyCaseCount, 43);
    ...
    assert.equal(toolPayload.summary.totalCaseCount, 19);
    assert.equal(toolPayload.summary.readyCaseCount, 19);
