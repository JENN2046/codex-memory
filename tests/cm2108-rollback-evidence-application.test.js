...
test('CM-2108 receipt contract rejects replay, native side effects, or completion overclaim', () => {
  ...
  for (const mutate of [
    ...
    payload => { payload.sourceRollbackReceipt.tree = '0'.repeat(40); },
    payload => { payload.sourceRollbackReceipt.bytes += 1; },
    payload => { payload.sourceRollbackReceipt.productionReady = true; },
    payload => { payload.contractResults.applicationGate = 'forged_gate'; },
    ...
    payload => { payload.applicationRuntime.unreviewedRuntimeClaim = false; },
    ...
    payload => { payload.boundaries.unreviewedCapability = false; }
  ]) {
