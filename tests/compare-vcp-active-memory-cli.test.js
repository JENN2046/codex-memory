...
test('compare active-memory CLI should run the repository standard suite in require-match mode', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.matchedCaseCount, 41);
    ...
    assert.equal(payload.categoryAggregate.find(item => item.category === 'input-validation')?.expectedErrorCaseCount, 7);
...
test('compare active-memory CLI should filter standard suite cases by expectation and tool', async () => {
  ...
  try {
    ...
    assert.equal(expectationReverseCounts.error, 17);
