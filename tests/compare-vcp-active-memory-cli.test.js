...
test('compare active-memory CLI should run the repository standard suite in require-match mode', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.matchedCaseCount, 34);
    assert.equal(payload.summary.mismatchedCaseCount, 0);
    assert.deepEqual(payload.summary.comparisonBreakdown, { matched: 34 });
    ...
    assert.equal(agentSelectionAggregate?.totalCaseCount, 5);
    assert.equal(agentSelectionAggregate?.fixtureAggregate?.find(item => item.fixture === 'vchat-fixture-multi-agent')?.totalCaseCount, 3);
    assert.equal(topicStateAggregate?.expectedErrorCaseCount, 3);
    assert.equal(payload.categoryAggregate.find(item => item.category === 'input-validation')?.expectedErrorCaseCount, 4);
...
test('compare active-memory CLI should filter standard suite cases by multiple categories', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.matchedCaseCount, 9);
    assert.equal(payload.summary.mismatchedCaseCount, 0);
...
test('compare active-memory CLI should filter standard suite cases by expectation and tool', async () => {
  ...
  try {
    ...
    assert.equal(expectationReverseCounts.error, 11);
    assert.equal(expectationReverseCounts.success, 23);
