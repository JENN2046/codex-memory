...
test('compare active-memory CLI should run the repository standard suite in require-match mode', async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.matchedCaseCount, 43);
    assert.equal(payload.summary.mismatchedCaseCount, 0);
    assert.deepEqual(payload.summary.comparisonBreakdown, { matched: 43 });
    ...
    assert.equal(payload.categoryAggregate.find(item => item.category === 'input-validation')?.expectedErrorCaseCount, 9);
...
test('compare active-memory CLI should filter standard suite cases by expectation and tool', async () => {
  ...
  try {
    ...
    assert.equal(expectationReverseCounts.error, 19);
