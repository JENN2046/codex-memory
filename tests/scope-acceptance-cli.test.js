...
test('scope-acceptance CLI should output status=ok with --json', () => {
  ...
  assert.deepEqual(report.dimensions, ['project_id', 'workspace_id', 'client_id', 'visibility']);
  ...
  assert.equal(report.projectA.scope.project_id, 'proj-alpha');
  assert.equal(report.projectA.scope.client_id, 'codex');
  assert.equal(report.projectA.scope.visibility, 'project');
  assert.ok(report.projectA.scope.workspace_id);
  ...
  assert.equal(report.projectB.scope.project_id, 'proj-beta');
  assert.equal(report.projectB.scope.client_id, 'claude');
  assert.equal(report.projectB.scope.visibility, 'shared');
  assert.ok(report.projectB.scope.workspace_id);
