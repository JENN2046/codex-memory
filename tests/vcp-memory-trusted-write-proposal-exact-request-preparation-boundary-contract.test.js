...
} = require('../src/core/VcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract');

function zeroCounters(overrides = {}) {
  return Object.fromEntries(ZERO_COUNTER_FIELDS.map(field => [field, overrides[field] ?? 0]));
}

function requestPreparationContract(overrides = {}) {
  const packet = {
    preparation_id: 'm9_fixture_trusted_write_proposal_exact_request_preparation_boundary_001',
    contract_version: 'vcp_memory_trusted_write_proposal_exact_request_preparation_boundary_v1',
  ...
  const evidence = {
    ...
    cm1831_packet_readiness_fixture_contract_present: true,
    cm1832_closeout_gate_review_present: true,
    local_packet_readiness_fixture_contract_closed: true
  };

  const preparation = {
    local_request_preparation_boundary_fixture_present: true,
    ...
    missing_exact_boundary_declared: true
  };

  const missingExactBoundary = Object.fromEntries(
    REQUIRED_MISSING_EXACT_BOUNDARY_FIELDS.map(field => [field, true])
  );
  ...
  const output = {
    disclosure_level: 'redacted_request_preparation',
  ...
  return {
    ...
    missingExactBoundary: {
      ...missingExactBoundary,
      ...(overrides.missingExactBoundary || {})
    },
    authorization: {
      ...authorization,
    ...
    expectedDecision: overrides.expectedDecision || 'request_preparation_blocked_missing_exact_boundary',
    nextActionAllowed: overrides.nextActionAllowed || 'cm1833_fixture_contract',
...
test('CM1833 accepts blocked exact request preparation boundary without runtime proposal or write', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract()
  );

  assert.equal(result.accepted, true);
  assert.equal(
    result.contractMode,
    'fixture_trusted_write_proposal_exact_request_preparation_boundary_only'
  );
  assert.equal(result.decision, 'request_preparation_blocked_missing_exact_boundary');
  assert.equal(result.exactRequestPreparationBoundaryAccepted, true);
  assert.equal(result.realExactRequestPacketPresent, false);
  assert.equal(result.exactRequestSubmissionReady, false);
...
test('CM1833 reports request_preparation_incomplete when evidence preparation or missing boundary declarations are absent', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      evidence: {
        cm1831_packet_readiness_fixture_contract_present: false,
        cm1832_closeout_gate_review_present: false
      },
      preparation: {
        local_request_preparation_boundary_fixture_present: false,
        missing_exact_boundary_declared: false
      },
      missingExactBoundary: {
        exact_payload_shape_missing: false,
        approval_line_value_missing: false
      },
      expectedDecision: 'request_preparation_incomplete'
    })
  );

  assert.equal(result.accepted, true);
  assert.equal(result.decision, 'request_preparation_incomplete');
  assert.equal(result.exactRequestPreparationBoundaryAccepted, false);
  assert.equal(result.realExactRequestPacketPresent, false);
  assert.equal(result.requestSubmitted, false);
  assert.equal(result.memoryWritten, false);
});

test('CM1833 stops exact request proposal runtime write and unlock claims as L4', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      preparation: Object.fromEntries(REQUIRED_PREPARATION_FIELDS.map(field => [field, true])),
...
test('CM1833 rejects raw secret request approval and readiness fields without echoing values', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      packet: {
      ...
      exactRequestSubmissionReady: 'SYNTHETIC_SUBMIT_READY_SHOULD_NOT_ECHO',
  ...
  assert.ok(result.forbiddenFields.includes('exactRequestSubmissionReady'));
  ...
  assert.equal(serialized.includes('SYNTHETIC_SUBMIT_READY_SHOULD_NOT_ECHO'), false);
...
test('CM1833 rejects unexpected fields without echoing submitted values', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      extraRoot: 'SYNTHETIC_EXTRA_ROOT_SHOULD_NOT_ECHO',
      preparation: {
        extraPreparation: 'SYNTHETIC_EXTRA_PREPARATION_SHOULD_NOT_ECHO'
      },
      missingExactBoundary: {
  ...
  assert.ok(result.unexpectedFields.includes('preparation.extraPreparation'));
  assert.ok(result.unexpectedFields.includes('missingExactBoundary.extraMissing'));
  assert.equal(serialized.includes('SYNTHETIC_EXTRA_ROOT_SHOULD_NOT_ECHO'), false);
  assert.equal(serialized.includes('SYNTHETIC_EXTRA_PREPARATION_SHOULD_NOT_ECHO'), false);
  assert.equal(serialized.includes('SYNTHETIC_EXTRA_MISSING_SHOULD_NOT_ECHO'), false);
});

test('CM1833 rejects missing positive and malformed zero side-effect counters', () => {
  const missingFixture = requestPreparationContract();
  delete missingFixture.counters.providerApiCalls;

  const missingResult = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    missingFixture
  );
  ...
  const positiveResult = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      counters: {
  ...
  const malformedResult = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      counters: {
        providerApiCalls: '0'
      }
  ...
  assert.equal(
    ...
    'invalid_trusted_write_proposal_exact_request_preparation_boundary_contract'
  );
  assert.ok(malformedResult.invalidFields.includes('counters.providerApiCalls'));
});

test('CM1833 rejects invalid packet fields decision mismatch and unsafe decision echo', () => {
  const unsafeExpectedDecision = 'PRIVATE_DECISION_SHOULD_NOT_ECHO';
  const invalid = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      packet: {
        preparation_id: 'unsafe_preparation_id',
  ...
  assert.equal(invalid.reasonCode, 'invalid_trusted_write_proposal_exact_request_preparation_boundary_contract');
  assert.ok(invalid.invalidFields.includes('packet.preparation_id'));
  ...
  const mismatch = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract({
      preparation: {
        exact_request_submission_ready: true
      },
      expectedDecision: 'request_preparation_blocked_missing_exact_boundary'
    })
  );
...
test('CM1833 locks request-preparation vocabulary and no-side-effect posture', () => {
  const result = validateVcpMemoryTrustedWriteProposalExactRequestPreparationBoundaryContract(
    requestPreparationContract()
  );

  assert.ok(ALLOWED_DECISIONS.includes('request_preparation_blocked_missing_exact_boundary'));
  assert.ok(ALLOWED_DECISIONS.includes('request_preparation_incomplete'));
  assert.ok(ALLOWED_DECISIONS.includes('stop_l4'));
  assert.ok(ALLOWED_NEXT_ACTIONS.includes('cm1833_fixture_contract'));
  assert.ok(REQUIRED_EVIDENCE_FIELDS.includes('cm1832_closeout_gate_review_present'));
  assert.ok(REQUIRED_PREPARATION_FIELDS.includes('exact_request_submission_ready'));
  assert.ok(REQUIRED_MISSING_EXACT_BOUNDARY_FIELDS.includes('approval_line_value_missing'));
  ...
  assert.ok(FORBIDDEN_FIELD_NAMES.includes('exactRequestSubmissionReady'));
