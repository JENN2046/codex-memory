  ...
  try {
    db.prepare(`
      INSERT INTO memory_records (
        ...
        ?, 'process', 'Validate runtime entry candidate', 'Type: checkpoint\nvalidate runtime entry candidate', 'seed evidence', '[]',
...
async function withApp(overrides = {}, handler) {
  const tempBasePath = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'codex-memory-validate-runtime-entry-'));
...
test('internal validate runtime entry is default-disabled and preserves current row', async () => {
  await withApp({}, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });

    const result = await app.executeInternalValidate(runtimeEntryPayload(), approvedRequestContext());
...
test('internal validate runtime entry rejects missing approved execution context', async () => {
  await withApp({
    internalValidateRuntimeEntryEnabled: true
  }, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });

    const result = await app.executeInternalValidate(runtimeEntryPayload(), {
      executionContext: {
...
test('internal validate runtime entry applies proposal record when enabled and approved', async () => {
  await withApp({
    internalValidateRuntimeEntryEnabled: true
  }, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });
...
test('public validate_memory valid dry-run reaches service preview without mutating', async () => {
  await withApp({}, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });

    const publicResult = await app.callTool('validate_memory', runtimeEntryPayload({
    ...
    assert.equal(publicResult.decision, 'rejected');
    assert.equal(publicResult.tool, 'validate_memory');
...
test('public validate_memory rejects dry_run=false before service mutation', async () => {
  await withApp({}, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });
...
test('public validate_memory rejects confirm=true before service mutation', async () => {
  await withApp({}, async ({ app }) => {
    insertRecord(app.config.dbPath, { memoryId: 'mem-1', status: 'proposal' });
