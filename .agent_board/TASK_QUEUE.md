...
| CM-1208 | 1208 | todo | P0-mainline-health | A5 approval-required runtime gate preflight | `docs/P66_A5_RUNTIME_GAP_CLOSURE_APPROVAL_PACKET.md`; `docs/CM1207_RUNTIME_GAP_CLOSURE_SCOPE_PREFLIGHT.md` | If user approves, run A5-GAP-5 cutover-context strict gate for fresh `HEAD` only | exact user approval line; fresh Git preflight; `npm run gate:mainline:strict` only if approved | no remote write; no config/watchdog/startup/provider/real-memory/durable-write action | yes, explicit A5 | Do not execute from generic "continue"; requires exact approval line with current commit. |
