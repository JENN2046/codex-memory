...
| CMB-0029 | open-runtime-refresh-approval-required | CM1537-live-proof-preflight-stale-runtime | P4-http-runtime / P8-memory-governance / P0-mainline-health | CM-1537 live client integration proof rerun after freshness guard | Exact proof rerun approval was provided and clean synced `main` was confirmed, but `npm run start:http:ensure` failed closed before proof requests because the live HTTP runtime was healthy while current-source runtime freshness evidence was missing or mismatched. No `initialize`, `tools/list`, or `tools/call` proof request was sent. | Obtain a separate exact decision before stopping/restarting/refreshing the local HTTP runtime or using another bounded remediation path. After runtime freshness matches current source on clean synced `main`, request/confirm the exact proof rerun boundary before executing no-bearer live MCP proof. | 2026-06-09 |
