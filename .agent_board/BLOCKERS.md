...
| CMB-0016 | open | no-token-selected-output-boundary | P4-http-runtime / P8-memory-governance / P9-codex-claude-client-scope | CM-1182 no-token memory_overview selected-output posture review | No-token `memory_overview` currently shares authorized overview projection and can expose paths, memory ids, titles, reasons, file/source references, recent files, and profile/provider details; there is no no-token selected projection. | Implement a separate source/test slice that either blocks no-token `memory_overview` or adds a strict selected no-token projection with HTTP response-shape tests. | 2026-05-26 |
