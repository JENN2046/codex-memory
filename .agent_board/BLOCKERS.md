...
| CMB-0027 | open-f4-exact-approval-required | Phase-F4-minimal-personal-dogfood-write-approval-missing | P9-codex-claude-client-scope / P8-memory-governance / P0-mainline-health | CM-1382 Phase F4 minimal dogfood write approval surface | F1/F2/F3 evidence are accepted, and CM-1382 prepares a dynamic snapshot-rendered F4 exact approval template. F4 minimal personal dogfood write has still not been approved or executed. | After CM-1382 is committed/synced, send the fresh exact approval line from `approvalTemplates.f4MinimalDogfoodWriteApprovalTemplate` only when `f4MinimalDogfoodWriteTemplateCurrentlyUsable=true`. Do not proceed to F5 until F4 evidence is accepted. No provider, `search_memory`, broad real memory scan, config/watchdog/startup change, remote action, readiness claim, or reliability claim without explicit scope. | 2026-06-02 |
