...
| CMB-0026 | open-f3-exact-execution-approval-required | Phase-F3-true-live-recall-negative-control-execution-approval-missing | P9-codex-claude-client-scope / P8-memory-governance / P0-mainline-health | CM-1380 Phase F3 true-live recall approval surface | F1 and F2 evidence are accepted, and CM-1380 prepares a dynamic snapshot-rendered F3 exact approval template. F3 execution is still blocked until CM-1380 is committed/synced, the fresh snapshot marks the F3 template currently usable, and the operator sends that exact line. | After commit/sync, send the fresh exact approval line from `approvalTemplates.f3TrueLiveRecallApprovalTemplate` before any F3 execution. Do not proceed to F4/F5 until F3 evidence is accepted. No provider, durable write, config/watchdog/startup change, broad real memory scan, remote action, readiness claim, or reliability claim without explicit scope. | 2026-06-02 |
