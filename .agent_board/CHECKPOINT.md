...
- Thirty-four focused R4 tests cover claim/ack, request and lease expiry, reconnect/reclaim, replay, in-flight cancellation, asynchronous completion state recheck, response correlation, loopback normalization, split UTF-8 preservation, non-loopback rejection, and body-log absence.
- Final default validation passed `5760` with `8` documented skips; hardening passed `97/97 + 6/6`.
