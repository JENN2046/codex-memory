...
| Current task | CM-1385 Phase F snapshot review fix |
...
| Runtime/source touched by current task | local snapshot CLI/test/docs/board only; no runtime mutation |
...
CM-1385 removes manual accepted-flag bypass from the Phase F snapshot CLI and cleans stale post-closeout operator text. After the guarded local commit containing this state, sync only with explicit approval; otherwise choose the next local phase from fresh Git facts. Do not execute memory tools, provider calls, remote actions, release/cutover actions, or broad readiness/reliability claims without separate scope.
