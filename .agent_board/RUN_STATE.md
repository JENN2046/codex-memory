...
| Git repository | yes |
| Branch | main |
| Local HEAD at CM-0722 start | `44d931b feat: expose governance blocker input placeholders` |
| Remote baseline | `origin/main = 4997db5 feat: add local autopilot control loop surfaces` |
| Worktree at CM-0722 start | `main...origin/main [ahead 19]`; tracked worktree was clean before this governance blocker text-summary slice. |
| Mode | Smart Standing Authorization v3 - Budgeted Autonomy Envelope |
| A4.8 role | legacy local-safe rail and Green Lane substrate |
| Current task | CM-0722 Dashboard governance blocker text summary; preserve `NOT_READY_BLOCKED / RC_NOT_READY_BLOCKED`. |
...
| Last action | Added read-only compact `GovBlk1..GovBlk5` text lines to dashboard output so each remaining governance blocker exposes command ids and missing artifact path requirements without JSON parsing or blocker decision changes. |
...
| Safe-push readiness | Not evaluated for CM-0704 through CM-0722; push still requires explicit user authorization or a separately passing safe-push packet. |
| Next planned action | Create a guarded local commit for CM-0722 if clean, then continue local-safe governance hardening or wait for explicit push authorization. |
