...
| Current task | CM-1311 candidate-cache governance entry id alias fallback completed/validated; commit state must be checked from fresh Git |
...
| Runtime/source touched by current task | `src/storage/CandidateCacheStore.js`; candidate-cache governance entries now fall through blank `memoryId` to `memory_id` before persistence/retrieval/sort |
...
CM-1311 fixes candidate-cache governance entry id fallback so blank `memoryId` does not mask effective `memory_id` before governance entries are persisted, retrieved, or sorted. It does not execute live write/recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted cache/sync tests passed `52/52`; default `npm test` passed `2837/2837`. Next safe action is to verify fresh Git state; if CM-1311 is already committed, continue to the next runtime gap, otherwise commit the validated scope.
