...
| Git repository | yes |
| Branch | main |
| Local HEAD at CM-0720 start | `41b5cab feat: expose governance blocker details` |
| Remote baseline | `origin/main = 4997db5 feat: add local autopilot control loop surfaces` |
| Worktree at CM-0720 start | `main...origin/main [ahead 17]`; tracked worktree was clean before this governance blocker command-hints slice. |
| Mode | Smart Standing Authorization v3 - Budgeted Autonomy Envelope |
| A4.8 role | legacy local-safe rail and Green Lane substrate |
| Current task | CM-0720 Dashboard governance blocker command hints; preserve `NOT_READY_BLOCKED / RC_NOT_READY_BLOCKED`. |
...
| Last action | Added read-only command hints to dashboard `governanceBlockerDetails` so each remaining governance blocker exposes `primaryCommand` and command preview usability without changing blocker decisions. |
