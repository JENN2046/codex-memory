...
| Current task | CM-1315 SQLite shadow record/chunk write memory-id alias normalization completed/validated; commit state must be checked from fresh Git |
...
| Runtime/source touched by current task | `src/storage/SqliteShadowStore.js`; shadow upsert/chunk replacement now uses normalized `memoryId/memory_id` |
...
CM-1315 fixes SQLite shadow-store record/chunk write id normalization so blank camel-case `memoryId` does not mask snake_case `memory_id` before `upsertRecord(...)` and `replaceChunksForRecord(...)`. It does not execute live write/recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted shadow/sync/storage tests passed `28/28`; default `npm test` passed `2841/2841`. Next safe action is to verify fresh Git state; if CM-1315 is already committed, continue to the next runtime gap, otherwise commit the validated scope.
