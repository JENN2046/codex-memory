...
| Current task | CM-1292 memory write visibility policy fallback normalization validated |
...
| Runtime/source touched by current task | `src/core/MemoryWriteService.js`; `record_memory` write-scope visibility now uses first non-empty normalized `visibility/visibility_policy` values |
