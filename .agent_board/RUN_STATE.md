...
| Current task | CM-1305 read policy result memory id alias fallback validated; local commit pending |
...
| Runtime/source touched by current task | `src/app.js`; search_memory post-filter read policy memory id lookup now falls through blank `memoryId` to `memory_id` |
...
CM-1305 fixes search_memory post-filter read policy result id fallback so blank `memoryId` does not mask `memory_id` before scope/lifecycle/governance/soft-read policy map lookup and stale counters. It does not execute true-live recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted lifecycle/policy tests passed `33/33`; default `npm test` passed `2829/2829`. Next safe action is docs/ledger validation, changed-scope review, and local commit if validation remains clean.
