...
| Current task | CM-1301 selected audit-correlation preflight alias fallback validated |
...
| Runtime/source touched by current task | `src/core/SelectedAuditCorrelationObservationPreflight.js`; selected audit-correlation preflight now normalizes blank camel-case packet/prior/artifact fields through snake_case aliases |
...
CM-1301 fixes selected audit-correlation observation preflight alias fallback so blank camel-case packet/request/prior-result/current-artifact fields do not mask snake_case aliases. It does not execute selected audit observation, read true audit logs, call providers/MCP, scan real memory, write durable memory/audit, change config/watchdog/startup, or claim readiness. Next safe action is default/docs validation, changed-scope review, and local commit if validation passes.
