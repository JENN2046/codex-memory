...
| Current task | CM-1286 rollback cleanup fallback normalization validated |
...
| Runtime/source touched by current task | `src/core/MemoryWriteRollbackCleanupDryRunPreview.js`; `src/core/MemoryWriteRollbackCleanupStoreBackedDryRunPreview.js`; `src/core/MemoryWriteRollbackCleanupApplyDesignPolicy.js`; rollback cleanup preview/apply-design memory ids and store kinds now use first non-empty normalized camel/snake values |
