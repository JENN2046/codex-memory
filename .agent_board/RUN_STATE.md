...
| Git repository | yes |
| Branch | main |
| Local HEAD at CM-0719 start | `6633a2a feat: expose recall scope readiness rollup` |
| Remote baseline | `origin/main = 4997db5 feat: add local autopilot control loop surfaces` |
| Worktree at CM-0719 start | `main...origin/main [ahead 16]`; tracked worktree was clean before this governance blocker details rollup slice. |
| Mode | Smart Standing Authorization v3 - Budgeted Autonomy Envelope |
| A4.8 role | legacy local-safe rail and Green Lane substrate |
| Current task | CM-0719 Dashboard governance blocker details rollup; preserve `NOT_READY_BLOCKED / RC_NOT_READY_BLOCKED`. |
...
| Last action | Added ordered read-only governance blocker details to dashboard `readinessSummary` so all remaining governance blockers are machine-readable without changing first-action semantics. |
