...
| Current task | CM-1322 mutation audit snapshot alias normalization completed/validated; commit state must be checked from fresh Git |
...
| Runtime/source touched by current task | `src/core/ValidateMemoryService.js`; `src/core/TombstoneMemoryService.js`; `src/core/SupersedeMemoryService.js`; mutation audit snapshots now use normalized `memoryId/memory_id` and `updatedAt/updated_at` |
...
CM-1322 fixes validate/tombstone/supersede lifecycle mutation audit snapshot normalization so blank camel-case `memoryId` / `updatedAt` in store/object-model records does not mask `memory_id` / `updated_at` before `previous_snapshot_ref` construction. It does not execute live write/recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted lifecycle mutation tests passed `59/59`; default `npm test` passed `2852/2852`. Next safe action is to verify fresh Git state; if CM-1322 is already committed, continue to the next runtime gap, otherwise commit the validated scope.
