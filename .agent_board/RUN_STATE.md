...
| Current task | CM-1314 vector-index memory-id alias normalization completed/validated; commit state must be checked from fresh Git |
...
| Runtime/source touched by current task | `src/storage/VectorIndexStore.js`; vector upsert/delete/score/diary rebuild now uses normalized `memoryId/memory_id` |
...
CM-1314 fixes vector-index memory-id alias normalization so blank camel-case `memoryId` does not mask snake_case `memory_id` before vector upsert, isolated-record vector deletion, explicit delete, score-map output, or diary-vector rebuild cache lookup. It does not execute live write/recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted recall/vector tests passed `52/52`; default `npm test` passed `2840/2840`. Next safe action is to verify fresh Git state; if CM-1314 is already committed, continue to the next runtime gap, otherwise commit the validated scope.
