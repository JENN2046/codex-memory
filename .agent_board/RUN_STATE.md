...
| Current task | CM-1288 supersede pair scope fallback normalization validated |
...
| Runtime/source touched by current task | `src/core/SupersedeMemoryService.js`; supersede pair scope guard now uses first non-empty normalized camel/snake values for project/workspace/client/task/conversation/visibility/retention record fields |
...
CM-1288 fixes `supersede_memory` pair scope guard fallback normalization so blank camel-case pair scope fields do not mask SQLite-style snake_case record fields before mutation/audit decisions. It preserves dry-run/no-audit/no-mutation behavior in temp-local tests and does not call providers/MCP, scan real memory, write durable memory/audit outside temp-local test stores, change config/watchdog/startup, expand public MCP tools, or claim readiness. Next safe action after this local stage is another local-safe Codex/Claude client isolation or runtime-governance source/test slice, or a fresh exact approval path for A5/Red-lane evidence.
