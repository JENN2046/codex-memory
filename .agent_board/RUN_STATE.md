...
| Current task | CM-1264 soft read policy client identity hardening added |
...
| Runtime/source touched by current task | `src/app.js` soft read policy privacy hardening |
...
CM-1264 hardens soft read policy client identity: private filtering now trusts `requestContext.executionContext` for caller identity and treats `scope.client_id` only as a candidate filter. It does not call providers/MCP, scan real memory, write durable memory/audit, change config/watchdog/startup, or claim readiness. Next safe action is final source/test/docs/ledger/diff validation, then commit or otherwise stabilize CM-1264.
