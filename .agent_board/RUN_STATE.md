...
| Current task | CM-1254 runtime truth table no-token overview rebase docs-only correction |
...
CM-1254 corrects stale docs-only status wording in `docs/CURRENT_RUNTIME_GAP_TRUTH_TABLE.md`: CM-1182 no-token `memory_overview` facts are now marked historical pre-CM-1183 behavior, while current source reality is the CM-1183 HTTP 403 / `NO_TOKEN_OVERVIEW_REJECTED` block before tool execution. It does not change runtime/source/test/config, run HTTP MCP, call provider/MCP, scan real memory, write durable memory/audit, or claim readiness. Next safe action is final docs/ledger/diff validation, then commit or otherwise stabilize CM-1254.
