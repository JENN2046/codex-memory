...
| Current task | CM-1316 chunk-indexing memory-id alias normalization completed/validated; commit state must be checked from fresh Git |
...
| Runtime/source touched by current task | `src/recall/ChunkIndexingService.js`; chunk id generation now uses normalized `memoryId/memory_id` |
...
CM-1316 fixes chunk-indexing id normalization so blank camel-case `memoryId` does not mask snake_case `memory_id` before chunk id generation and shadow chunk replacement. It does not execute live write/recall, read real memory/store/jsonl, call providers/MCP, change config/watchdog/startup, expand public MCP, or claim readiness. Targeted chunk/write/restart tests passed `45/45`; default `npm test` passed `2842/2842`. Next safe action is to verify fresh Git state; if CM-1316 is already committed, continue to the next runtime gap, otherwise commit the validated scope.
