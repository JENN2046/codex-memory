...
| Current task | CM-1250 schema-gated startup recovery policy preflight implemented and targeted-validated |
...
| Runtime/source touched by current task | `src/core/MemoryWriteReconcileStartupSafetyPolicy.js` schema-gated preflight only |
...
CM-1250 connects the CM-1249 SQLite schema startup hard gate to startup recovery policy preflight: `buildStartupRecoverySafetyPreflight(...)` now requires accepted sanitized `shadowHealth.schemaStartupGate` and fail-closes absent, blocked, malformed, unaccepted, or future-versioned schema gate state. It does not execute recovery/apply, install startup/watchdog, change config, run HTTP MCP, apply migration/import/export/backup/restore, or claim readiness. Next safe action is final docs/ledger/diff validation, then commit or otherwise stabilize CM-1250.
