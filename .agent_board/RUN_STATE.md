...
| Current task | CM-1357 Phase F1 fresh sync/live preflight recorded; live-client execution requires explicit sync route plus exact A5-GAP-4 approval |
...
| Runtime/source touched by current task | none in CM-1357; docs/board preflight only |
...
CM-1357 records that F1 must use fresh Git facts before any sync or live execution. The preflight-time snapshot showed `main` ahead of `origin/main` by two commits, but this file must not be treated as current Git truth after later commits. Do not rely on old embedded approval packets. Next safe action is either explicit normal non-force push authorization followed by fresh synced-head A5-GAP-4 approval, or staying local and preparing only non-live Phase F work. Before any live recall proof, dogfood write, strict gate refresh, or RC work, require the separate exact boundary for that action.
