...
source_sha256_at_creation: f85b1e19ab2da059bfbf2c29d09d0093f6a59618248522123d5b52ab42738b70
...
## 5. Master / Stage / Version 治理链

本项目采用 Jenn 刚指定的 ColaMeta 治理链：

1. Master 定最终方向。
   对 ColaMeta 自身，产品级模式是：`ColaMeta 最终要成为目标锚定的 AI 交付指挥层。`
   对本 `codex-memory` Master，本项目最终方向是第 3 节的北极星：通过 `codex-memory`
   governance bridge，让 Codex / Claude 有治理地使用 VCPToolBox native memory。
2. Stage 定阶段目标。
   每个 Stage 必须说明自己服务哪个 Master goal、为什么服务它、明确不做什么，以及用什么
   证据证明自己没有漂移。
3. Version 定本轮任务。
   每个 Version 必须引用 Master 和 Stage，只交付一个具体小能力，不能自己扩展 project final goal。
4. Execution Envelope 锁执行边界。
   Agent 只能改 `allowed_files`，必须遵守 `forbidden_files` 和 `out_of_scope`，只能跑绑定的
   validation commands 或另行授权的安全验证。
5. Review 再问一次。
   closeout 问题是：`这次 work 是否仍服务 project_final_goal？` 如果答案不确定，必须进入
   review 或 plan adjustment，不能靠假设标记完成。

任何 Stage、Version、Execution Envelope、review packet、ColaMeta preview 或 executor report，
都不能静默替换 Master final direction。
