...
const { structuredContentFromToolResult } = require('./html');
...
function parseToolResultNotification(message) {
  ...
  const structuredContent = structuredContentFromToolResult(message.params);
  if (!structuredContent) reject('widget_tool_result_notification_invalid');
