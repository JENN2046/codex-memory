...
Targeted tests pass `35/35`, the default suite passes `5840/0/8`, and
hardening passes `97/97 + 6/6`. `gate:mainline:strict` passes its offline
contract (`112/112`), test (`5840/0`), compare (`43/43`), and rollback
...
The CI-safe gate passes `5931/0/9` with `noNetwork`, `noDaemon`, and
`noProvider` all true.
