...
| `mainline_strict_gate_not_executed_for_cutover` | P66.53-P66.55 local proof slice is complete. | No cutover-context `gate:mainline:strict` execution or branch-sensitive cutover gate performed. | A5/cutover-context preflight | No | Run a fresh cutover-context strict gate only under explicit cutover/preflight authorization. |
