...
| `migration_import_export_backup_restore_approval_execution_blocked` | P66.47-P66.49 local proof slice is complete; [A5-GAP-3 dry-run approval/execution packet](/A:/codex-memory/docs/P66_A5_GAP_3_DRY_RUN_APPROVAL_EXECUTION_PACKET.md) narrows the next approval to `vcp-memory:migration-readiness` fixture-only dry-run. | No migration/import/export/backup/restore apply or approval execution performed. | A5 dry-run approval pending | No | If approved, execute only `npm run vcp-memory:migration-readiness -- --json` and record `mutated=false` / `migrationBlocked=true`; keep apply/import/export/backup/restore/durable write blocked. |
