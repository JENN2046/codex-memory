...
| live HTTP operation readiness | Endpoint-bound HTTP evidence exists for loopback `7605`, including health/MCP/observe evidence, with no config/watchdog/startup change. Long-running HTTP session hardening remains a production-hardening gap. | yes, endpoint-bound observation only | A5 endpoint-bound evidence | no | Add a local-safe HTTP session TTL/cap/cleanup design packet, then implement only after a scoped runtime plan is approved. |
