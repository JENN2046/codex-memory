...
CM-1943 reviews the CM-1942 startup failure diagnosis request packet and
displays the exact boundary Jenn would need to approve before any future live
startup failure diagnosis execution.
...
runtime, restart runtime, recheck listener reachability, resolve or disclose an
endpoint or locator value, read config/env, read secrets, read logs, read
stdout/stderr, read response bodies, read raw error payloads, read raw memory,
write memory, change configuration, change startup/watchdog state, expand
public MCP, push, release, deploy, cut over, or claim readiness.

## Readiness Review Decision

```yaml
decision:
  cm1942_packet_present: true
  cm1942_packet_ready_for_jenn_boundary_display: true
  cm1941_startup_failure_contract_required: true
  ...
  listener_after_start_known: false
  target_locator_binding_success_known: false
  component_action_status_probe_unlocked: false
  read_shape_unlocked: false
  readiness_claimed: false
  next_route: cm1944_exact_approval_capture_pre_execution_gate
```

CM-1943 can display the boundary because CM-1942 defines a concrete
non-authorizing packet and CM-1941 defines the required local startup failure
diagnosis contract. That is enough for Jenn review. It is not enough for
runtime execution.
...
- explicit and exact
- bounded to purpose `runtime_startup_failure_diagnosis`
- bounded to safe target reference `operator-vcp-toolbox-service-ref`
- bounded to the diagnostic lanes listed above
- bounded to CM-1941 `VcpNativeStartupFailureDiagnosisContract`
...
- bounded to no stdout/stderr, config/env, secret, memory read, memory write,
  durable write, endpoint disclosure, locator value disclosure, command-line
  disclosure, process identifier disclosure, provider/API call, public MCP
  expansion, config/startup/watchdog change, dependency change,
...
- non-retry after the single approved live/network startup failure diagnosis
  attempt is consumed
...
The following remain blocked after CM-1943:

- live startup failure diagnosis execution
- retry of CM-1938
...
- command-line or process identifier disclosure
...
CM-1943 validation is docs/status/governance only:
...
Next route is CM-1944 exact approval capture / pre-execution gate.

CM-1944 must still be non-executing unless Jenn has already provided a separate
current exact approval matching the CM-1942 / CM-1943 boundary. If that approval
is missing, stale, broader than the boundary, or ambiguous, CM-1944 must stop
before runtime.
