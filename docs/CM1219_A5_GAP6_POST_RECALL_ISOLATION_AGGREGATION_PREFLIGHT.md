# CM-1219 A5-GAP-6 Post-Recall-Isolation Aggregation Preflight
...
Prepare the next exact-approved `A5-GAP-6` aggregation refresh after CM-1218 added current no-mutation `A5-GAP-2` recall isolation evidence.

This document is not authorization and does not execute ValidationAggregator. It fixes the next evidence-consumption boundary so the project can aggregate the current approved runtime evidence set without scanning stores, invoking MCP tools, or claiming readiness.
...
```text
branch = main
HEAD = 840556d7c7be1ddf6172a890fa87193eee9fbd6f
...
```text
A5-GAP-1,A5-GAP-2,A5-GAP-4,A5-GAP-5
```
...
Historical `A5-GAP-3` artifacts may be background context only. They must not be consumed by the next aggregation run unless the exact approval line explicitly names `A5-GAP-3`.

## Allowed Future Aggregation Shape

The next `A5-GAP-6` execution, if separately approved, should:

- feed only explicitly produced sanitized evidence from approved units `A5-GAP-1,A5-GAP-2,A5-GAP-4,A5-GAP-5` into ValidationAggregator
...
```text
I approve A5-GAP-6 for codex-memory on branch main at commit <FRESH_HEAD>, using only evidence from approved A5-GAP units A5-GAP-1,A5-GAP-2,A5-GAP-4,A5-GAP-5.
```
...
```text
cm1219_preflight_created=true
a5_gap6_executed=false
validation_aggregator_executed=false
selected_units=A5-GAP-1,A5-GAP-2,A5-GAP-4,A5-GAP-5
historical_gap_3_consumed=false
runtime_ready=false
rc_ready=false
decision=NOT_READY_BLOCKED
```
