# CM-1212 A5-GAP-6 Aggregation Refresh Preflight
...
Prepare the next exact-approved `A5-GAP-6` evidence aggregation refresh after the current `A5-GAP-5` strict-gate and `A5-GAP-4` HTTP/MCP evidence records.

This document is not authorization and does not execute the ValidationAggregator. It records the approval boundary that must be used before any aggregation refresh.
...
```text
branch = main
HEAD = 7d9db9a2296b1c5b9199d2f3164eabe18c22d74f
branch state = main...origin/main [ahead 5]
untracked left untouched = CLAUDE.md, docs/CURRENT_FACTS_SINGLE_SOURCE_PLAN.md
```

Current Git facts must be rechecked immediately before execution. If this preflight is committed first, the approval line must bind to the new fresh `HEAD`, not the preflight snapshot above.
...
| `A5-GAP-4` | `CM-1210` endpoint-bound HTTP evidence | `main@db5a4d66cf472d35e80b12d512816cda5de09220`, endpoint `http://127.0.0.1:7605` | `/health` and `observe:http` passed; unauthenticated MCP failed closed with Unauthorized. |
...
Historical `A5-GAP-1`, `A5-GAP-2`, and `A5-GAP-3` artifacts may be background context only. They must not be consumed by the next aggregation run unless the exact approval line explicitly names them.

## Allowed Future Aggregation Shape

The next `A5-GAP-6` execution, if separately approved, should:

- feed only explicitly produced sanitized evidence from the approved unit list into ValidationAggregator
...
```text
I approve A5-GAP-6 for codex-memory on branch main at commit <FRESH_HEAD>, using only evidence from approved A5-GAP units A5-GAP-4,A5-GAP-5.
```
...
```text
cm1212_preflight_created=true
a5_gap6_executed=false
validation_aggregator_executed=false
selected_units=A5-GAP-4,A5-GAP-5
historical_gap_1_2_3_consumed=false
runtime_ready=false
rc_ready=false
decision=NOT_READY_BLOCKED
```
