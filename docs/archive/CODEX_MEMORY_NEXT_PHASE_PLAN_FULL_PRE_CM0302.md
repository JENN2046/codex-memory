# codex-memory Next Phase Plan - full pre-CM0302 archive
...
- P27.1 migration/import-export approval packet fixture shape: `tests/fixtures/migration-import-export-approval-packet-v1.json` and `tests/migration-import-export-approval-packet-fixture.test.js` lock the approval packet shape, blocked status, required approvals, safety flags, required wording, forbidden claims, and public MCP freeze.
- P27.2 direct-node fixture-only approval-packet CLI plan: P27 now names future candidate files `src/cli/migration-import-export-approval-packet.js` and `tests/migration-import-export-approval-packet-cli.test.js`, source mode `fixture`, future output contract, rejected unsafe flags, validation plan, and stop conditions. P27.2 is docs-only and does not implement the CLI, add package scripts, scan real memory, run apply, create/restore backups, start services, call providers, expand public MCP, push, tag, release, or deploy.
- P27.3 fixture-only approval-packet CLI implementation review: P27 now reviews the future P27.4 source/test slice, exact candidate files, allowed fixture-only behavior, still-disallowed actions, and implementation preconditions. P27.3 is docs/status/board only and does not implement the CLI, add tests, add package scripts, scan real memory, run apply, create/restore backups, start services, call providers, expand public MCP, push, tag, release, or deploy.
- P27.4 fixture-only approval-packet CLI: `src/cli/migration-import-export-approval-packet.js` and `tests/migration-import-export-approval-packet-cli.test.js` add a direct-node fixture-only CLI, `--json` report output, fail-closed unsafe-flag JSON, public MCP three-tool freeze, no-side-effect safety checks, and no raw secret/workspace exposure checks. No package script, real memory scan/export/import, SQLite migration apply, import/export apply, backup/restore, durable write, provider call, service startup, public MCP expansion, or push/tag/release/deploy occurred.
- P27.5 validation aggregator evidence shape: `src/core/ValidationAggregatorService.js`, `tests/fixtures/v1-rc-validation-aggregator-v1.json`, and aggregator tests now record P27.4 approval-packet CLI evidence with `cliExecuted=false`, `executionApproved=false`, `realMemoryScanned=false`, `backupRestorePerformed=false`, `durableReportWritten=false`, and `packageScriptAdded=false`. No aggregator CLI execution, package script, real memory scan/export/import, SQLite migration apply, import/export apply, backup/restore, durable write, provider call, service startup, public MCP expansion, or push/tag/release/deploy occurred.
- P27.x closeout review: [docs/P27_MIGRATION_IMPORT_EXPORT_APPROVAL_PACKET_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P27_MIGRATION_IMPORT_EXPORT_APPROVAL_PACKET_CLOSEOUT_REVIEW.md) closes P27 as approval-boundary documentation, fixture contract, fixture-only CLI, and aggregator report-shape evidence. It explicitly does not approve or prove real migration/import-export readiness.
- P28/P29 validation evidence and schema/version boundary chain: P28 adds explicit safe validation evidence reader summaries in `ValidationAggregatorService`; P29 adds the explicit-input `SchemaVersionPolicy` helper, evaluation report, schema runtime boundary guard test, and ValidationAggregator report-shape evidence for those surfaces. The chain remains fixture-first/read-only/report-only and keeps `schemaVersionRuntimeEnforcementImplemented=false`.
- P29 closeout review: [docs/P29_SCHEMA_VERSION_RUNTIME_ENFORCEMENT_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P29_SCHEMA_VERSION_RUNTIME_ENFORCEMENT_CLOSEOUT_REVIEW.md) closes P29 as fixture-backed, explicit-input, and report-shape evidence. It explicitly does not implement runtime enforcement, add public MCP schema fields, scan real memory, mutate durable state, or approve push/tag/release/deploy.
- P30 final RC validation matrix runner safe-scope inventory: [docs/P30_FINAL_RC_VALIDATION_MATRIX_RUNNER_SAFE_SCOPE_INVENTORY.md](/A:/codex-memory/docs/P30_FINAL_RC_VALIDATION_MATRIX_RUNNER_SAFE_SCOPE_INVENTORY.md) defines what a future runner may consume: explicit/local evidence, committed fixture/report-shape evidence, P23/P24/P28/P29 provenance, public MCP freeze, and safety fields. It does not implement or execute the runner.
- P30.1 final RC validation matrix runner fixture contract: `tests/fixtures/final-rc-validation-matrix-runner-safe-scope-v1.json` and `tests/final-rc-validation-matrix-runner-safe-scope-fixture.test.js` lock the future runner manifest boundary: explicit safe inputs only, no runner implementation, no final RC matrix execution, public MCP freeze, no side effects, fail-closed rejection defaults, and A4/conditional-live/runtime-required/A5 split.
- P30.2 final RC validation matrix runner manifest helper: `src/core/FinalRcValidationMatrixManifest.js` and `tests/final-rc-validation-matrix-runner-manifest-helper.test.js` summarize an already-provided manifest object without reading files or executing a runner. It keeps `canExecuteRunner=false`, `canClaimFinalRcReady=false`, `decision=NOT_READY_BLOCKED`, public MCP freeze, runtime blockers, A5 blockers, no-side-effect safety, and `acceptedSourceTypes` whitelist enforcement visible.
- P30.3 final RC validation matrix aggregator evidence shape: `ValidationAggregatorService` now surfaces P30.2 manifest-helper capability as static report-shape evidence only. The aggregator does not execute/import the helper, read the P30 fixture, implement a runner, or execute the final RC matrix, and it preserves `canExecuteRunner=false`, `canClaimFinalRcReady=false`, runtime blockers, A5 blockers, and public MCP freeze.
- P30.x closeout review: [docs/P30_FINAL_RC_VALIDATION_MATRIX_RUNNER_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P30_FINAL_RC_VALIDATION_MATRIX_RUNNER_CLOSEOUT_REVIEW.md) closes P30 as safe-scope, fixture-backed, explicit-input, helper-backed, and report-shape evidence. It explicitly does not implement a runner or execute the final RC matrix.
- P31 memory governance safe-scope inventory: [docs/P31_MEMORY_GOVERNANCE_SAFE_SCOPE_INVENTORY.md](/A:/codex-memory/docs/P31_MEMORY_GOVERNANCE_SAFE_SCOPE_INVENTORY.md) opens the next governance phase as docs/status/board inventory only. It maps existing lifecycle, controlled-write, mutation-audit, object-model, internal validate, and admin-review artifacts to proposal, supersession, tombstone, validation, audit, and approval boundaries without mutating durable memory.
- P31.1 memory governance fixture contract review: [docs/P31_1_MEMORY_GOVERNANCE_FIXTURE_CONTRACT_REVIEW.md](/A:/codex-memory/docs/P31_1_MEMORY_GOVERNANCE_FIXTURE_CONTRACT_REVIEW.md) reviews existing lifecycle, controlled-write, proposal-review, mutation-audit, validate-memory-runtime, and object-model fixtures. The smallest missing contract is a synthetic `memory-governance-lifecycle-contract-v1` fixture tying proposal accept/reject, supersession, tombstone, audit, approval posture, safe source types, and public MCP freeze into one fail-closed contract.
- P31.2 memory governance lifecycle contract fixture: `tests/fixtures/memory-governance-lifecycle-contract-v1.json` and `tests/memory-governance-lifecycle-contract-fixture.test.js` add a synthetic committed fixture/test for proposal accept/reject, supersession deferred, tombstone deferred, internal validate_memory, mutation audit, approval posture, safe source type whitelist, public MCP freeze, no-side-effect safety, and `NOT_READY_BLOCKED`.
- P31.3 memory governance lifecycle contract helper: `src/core/MemoryGovernanceLifecycleContract.js` and `tests/memory-governance-lifecycle-contract-helper.test.js` summarize a caller-provided governance lifecycle contract object without reading files or executing commands. It fails closed for unsupported source types, runtime/public MCP readiness claims, missing required surfaces, and missing lifecycle cases.
- P31.4 memory governance aggregator evidence shape: `src/core/ValidationAggregatorService.js`, `tests/fixtures/v1-rc-validation-aggregator-v1.json`, and aggregator tests now surface P31.3 helper capability as static report-shape evidence only. The aggregator does not import/execute the helper, read the P31 fixture, integrate runtime governance, expand public MCP, scan real memory, or mutate durable state.
- P31.x memory governance safe-scope closeout review: [docs/P31_MEMORY_GOVERNANCE_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P31_MEMORY_GOVERNANCE_CLOSEOUT_REVIEW.md) closes P31 only as safe-scope / fixture / explicit-input helper / report-shape evidence. Runtime governance integration, public MCP expansion, durable mutation, real memory scan/export/import, migration-import-export apply, backup/restore, provider/service/config actions, package scripts, runner execution, and `push` remain separately authorized only.
- P32 memory governance approval-packet safe-scope inventory: [docs/P32_MEMORY_GOVERNANCE_APPROVAL_PACKET_SAFE_SCOPE_INVENTORY.md](/A:/codex-memory/docs/P32_MEMORY_GOVERNANCE_APPROVAL_PACKET_SAFE_SCOPE_INVENTORY.md) defines future approval packet requirements for proposal accept/reject, supersession, tombstone, internal validation, audit/report writes, public MCP expansion, real-memory governance preview, backup, and restore. It is docs/status/board only and does not approve or execute any governed action.
- P32.1 memory governance approval-packet fixture contract: `tests/fixtures/memory-governance-approval-packet-v1.json` and `tests/memory-governance-approval-packet-fixture.test.js` add a synthetic committed fixture/test for governed action approval packet shape, safe source types, required fields, blockers, no-side-effect safety, public MCP freeze, and internal-only `validate_memory`.
- P32.2 memory governance approval-packet explicit-input helper: `src/core/MemoryGovernanceApprovalPacketContract.js` and `tests/memory-governance-approval-packet-helper.test.js` summarize caller-provided approval packet objects without reading fixture files or executing commands. It fails closed for unsupported source types, execution/readiness/runtime claims, missing required packet fields/actions, and unblocked governed actions.
- P32.3 ValidationAggregator approval-packet evidence shape: `src/core/ValidationAggregatorService.js`, `tests/fixtures/v1-rc-validation-aggregator-v1.json`, and aggregator tests now surface P32.2 helper capability as static report-shape evidence only. The aggregator does not import/execute the helper, read the P32 fixture, approve governed actions, integrate runtime governance, expand public MCP, scan real memory, or mutate durable state.
- P32.x memory governance approval-packet closeout review: [docs/P32_MEMORY_GOVERNANCE_APPROVAL_PACKET_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P32_MEMORY_GOVERNANCE_APPROVAL_PACKET_CLOSEOUT_REVIEW.md) closes P32 only as safe-scope / fixture / explicit-input helper / report-shape evidence. Runtime governance integration, public MCP expansion, durable mutation, real memory scan/export/import, migration-import-export apply, backup/restore, provider/service/config actions, package scripts, runner execution, and `push` remain separately authorized only.
- P33 memory governance audit-evidence safe-scope inventory: [docs/P33_MEMORY_GOVERNANCE_AUDIT_EVIDENCE_SAFE_SCOPE_INVENTORY.md](/A:/codex-memory/docs/P33_MEMORY_GOVERNANCE_AUDIT_EVIDENCE_SAFE_SCOPE_INVENTORY.md) inventories future governance audit evidence for proposal, supersession, tombstone, validation, approval, backup, restore, report-write, and public MCP expansion review actions using docs/fixture-first inputs only. It does not implement an audit writer or approve execution.
- P33.1 memory governance audit-evidence fixture contract: `tests/fixtures/memory-governance-audit-evidence-v1.json` and `tests/memory-governance-audit-evidence-fixture.test.js` add a synthetic committed fixture/test for audit evidence event families, required evidence fields, safe source types, blockers, no-side-effect safety, public MCP freeze, and internal-only `validate_memory`.
- P33.2 memory governance audit-evidence explicit-input helper: `src/core/MemoryGovernanceAuditEvidenceContract.js` and `tests/memory-governance-audit-evidence-helper.test.js` summarize caller-provided audit evidence contract objects without reading fixture files or executing commands. It fails closed for unsupported source types, audit writer/runtime/readiness claims, missing required evidence fields/event families, and unblocked event family records.
- P33.3 ValidationAggregator audit-evidence helper evidence shape: `src/core/ValidationAggregatorService.js`, `tests/fixtures/v1-rc-validation-aggregator-v1.json`, and aggregator tests now surface P33.2 helper capability as static report-shape evidence only. The aggregator does not import/execute the helper, read the P33 fixture, implement an audit writer, write durable audit logs, integrate runtime governance, expand public MCP, scan real memory, or mutate durable state.
- P33.x memory governance audit-evidence closeout review: [docs/P33_MEMORY_GOVERNANCE_AUDIT_EVIDENCE_CLOSEOUT_REVIEW.md](/A:/codex-memory/docs/P33_MEMORY_GOVERNANCE_AUDIT_EVIDENCE_CLOSEOUT_REVIEW.md) closes P33 only as safe-scope / fixture / explicit-input helper / aggregator report-shape evidence. Audit writer implementation, durable audit writes, durable mutation, public MCP expansion, real memory scans, provider/service/config actions, migration-import-export apply, backup/restore, package scripts, runner execution, and `push` remain separately authorized only.
- P34 governance review surface safe-scope inventory: [docs/P34_GOVERNANCE_REVIEW_SURFACE_SAFE_SCOPE_INVENTORY.md](/A:/codex-memory/docs/P34_GOVERNANCE_REVIEW_SURFACE_SAFE_SCOPE_INVENTORY.md) inventories a future read-only review surface over lifecycle, approval-packet, audit-evidence, P19 admin-review, and ValidationAggregator report-shape evidence. It does not implement a CLI, execute `governance:report` against the real configured database, approve governed actions, write durable audit/memory records, expand public MCP tools, or claim runtime readiness.
- P34.1 governance review surface fixture contract: `tests/fixtures/memory-governance-review-surface-v1.json` and `tests/memory-governance-review-surface-fixture.test.js` lock the future review surface shape over lifecycle, approval, audit-evidence, admin-review, and ValidationAggregator report-shape sources. The fixture is synthetic and proves no CLI implementation, no real DB review execution, no governed-action approval, no durable writes, no public MCP expansion, and no RC-ready claim.
- P34.2 governance review surface explicit-input helper: `src/core/MemoryGovernanceReviewSurfaceContract.js` and `tests/memory-governance-review-surface-helper.test.js` summarize caller-provided review surface objects only. The helper keeps `sourceMode=explicit_input`, fails closed for unsupported source types, runtime/readiness/approval claims, missing required review surfaces/sections/blockers/approvals, and unsafe safety flags, and redacts sensitive `authorization` / `bearer` / `api_key` / `raw_workspace_id` fragments. It does not read fixtures, execute commands, run `governance:report`, review a real DB, write durable state, expand public MCP, or claim RC readiness.
- P34.3 ValidationAggregator review-surface evidence shape: `src/core/ValidationAggregatorService.js`, `tests/fixtures/v1-rc-validation-aggregator-v1.json`, and aggregator tests now expose P34.2 helper capability as static report-shape evidence only. Aggregator does not import/execute the helper, read P34 fixtures, run `governance:report`, perform real DB review, approve governed actions, write durable audit/memory, expand public MCP, or claim runtime/RC readiness.
- CM-0301 active-board context compression: full pre-compression `.agent_board/CHECKPOINT.md` and `.agent_board/HANDOFF.md` were moved to `.agent_board/archive/`, while active files now keep only current state, boundaries, validation, and next-step pointers. This is docs/board-only context hygiene to reduce stale handoff pollution.
- Next recommended phase: continue a small docs/status stale-entry compression pass if high value, otherwise `P34.x-governance-review-surface-closeout-review`. Both should remain docs/status/board only if implemented.
