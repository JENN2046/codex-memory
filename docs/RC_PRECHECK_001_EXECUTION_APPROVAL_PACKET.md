...
Prepared target commit at last packet refresh: `86d495ab48158c83e88592181f8647da39651321`

Remote baseline: origin/main = 103c3ac

Endpoint for future HTTP evidence, if approved: http://127.0.0.1:7605

Execution-time target rule: before any approved full precheck execution, re-read `git rev-parse HEAD`; if `HEAD` is no longer `86d495ab48158c83e88592181f8647da39651321`, update this packet and approval line before running any A5 command.
...
`	ext
I approve A5-RC-PRECHECK-READONLY for target commit 86d495ab48158c83e88592181f8647da39651321 on local main, endpoint http://127.0.0.1:7605, using only: Git baseline, npm run gate:mainline:strict, npm run observe:http -- --json, npm run compare-active-memory -- --suite .\benchmarks\active-memory-suite\standard-suite.json --json --require-match, npm run rollback-active-memory -- --suite .\benchmarks\active-memory-suite\standard-suite.json --json --require-ready, public MCP freeze confirmation, and remaining runtime gaps confirmation. This approval does not authorize recall path observation, provider calls, real memory broad scans, migration/import/export/backup/restore apply, config/watchdog/startup changes, public MCP expansion, durable memory writes, push, tag, release, deploy, RC cutover, A5-GAP-7, or any readiness claim.
`
...
`	ext
I approve A5-RC-PRECHECK-RECALL for target commit 86d495ab48158c83e88592181f8647da39651321 on local main, using exactly one named recall subject/query and bounded recall audit observation to be written into the execution packet before running. This approval does not authorize provider calls, real memory broad scans beyond the named recall path, migration/import/export/backup/restore apply, config/watchdog/startup changes, public MCP expansion, durable memory writes outside the explicitly approved recall/audit path, push, tag, release, deploy, RC cutover, A5-GAP-7, or any readiness claim.
`
