...
CM-1931 reviews the CM-1930 startup / locator diagnosis request packet and
displays the exact boundary Jenn would need to approve before any future live
startup / locator diagnosis execution.
...
execute runtime, call VCPToolBox, inspect process state, resolve or disclose an
endpoint or locator value, read config/env, read secrets, read logs, read
response bodies, read raw memory, write memory, change configuration, or claim
readiness.
...
```yaml
decision:
  cm1930_packet_present: true
  cm1930_packet_ready_for_jenn_boundary_display: true
  cm1928_receipt_capture_contract_required: true
  ...
  next_route: cm1932_exact_approval_capture_pre_execution_gate
```

CM-1931 can display the boundary because CM-1930 already defines a concrete
non-authorizing packet and CM-1928 already defines the required deterministic
receipt-capture contract for future injected outcomes. That is enough for Jenn
review. It is not enough for runtime execution.
...
If Jenn later gives a separate exact approval matching this boundary, the future
...
- explicit and exact
- bounded to purpose `runtime_startup_or_target_locator_diagnosis`
- bounded to safe target reference `operator-vcp-toolbox-service-ref`
- bounded to the diagnostic lanes listed above
- bounded to one network call, one runtime call, and one process-state
  inspection
...
- bounded to no stdout/stderr, config/env, secret, memory read, memory write,
  ...
  call, public MCP expansion, release/deploy/cutover/push, or readiness claim
- bounded to the low-disclosure receipt projection only
- stopped immediately if any requested action exceeds this boundary
- non-retry after the single approved live/network attempt is consumed
...
The following remain blocked after CM-1931:

- live startup / locator diagnosis execution
- retry of CM-1926
...
CM-1931 validation is docs/status/governance only:
...
Next route is CM-1932 exact approval capture / pre-execution gate.

CM-1932 must still be non-executing unless Jenn has already provided a separate
current exact approval matching the CM-1930 / CM-1931 boundary. If that approval
is missing, stale, broader than the boundary, or ambiguous, CM-1932 must stop
before runtime.
