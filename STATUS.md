...
- 当前任务：P58-T1 migration/import-export/backup-restore approval framework boundary inventory 已完成、验证并本地提交为 `5326169`；board/status 已切到 P58-T2 explicit-input helper 候选。必须继续禁止服务/provider/真实 memory/A5 操作。
...
1. 评估 P58-T2 approval framework explicit-input helper 是否可作为下一条安全本地任务进入；不得 push，除非用户单独明确授权。
2. P58-T2 只有在保持 caller-provided object only、无 fs/command/durable write/runtime/provider/public MCP expansion 时才可进入。
