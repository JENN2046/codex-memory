...
- P12.1 controlled write fixture schemas 已 tests/docs-only 本地完成：新增 `tests/fixtures/controlled-write-tools-v1.json` 与 `tests/controlled-write-tools-fixture.test.js`，锁住候选工具 schema、mutation boundary、audit shape 和 dry-run-first 规则；不改 runtime，不新增 MCP tools，不写真实 DB。
