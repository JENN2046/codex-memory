...
- 当前 CM-0564 recall audit abort side-effect fixture：已新增 [CM-0564 recall audit abort side-effect fixture](/A:/codex-memory/docs/CM-0564_RECALL_AUDIT_ABORT_SIDE_EFFECT_FIXTURE.md)。该切片只新增 fixture-only test，验证 synthetic `KnowledgeBaseRecallPipeline` 在 abort signal 后返回 `SEARCH_MEMORY_TIMEOUT` 且不会调用 `recallAuditService.record`。验证为 `node --check .\tests\recall-isolation-classification-runtime.test.js` 通过、`node --test .\tests\recall-isolation-classification-runtime.test.js` 7/7 通过、`node --test .\tests\mcp-contract.test.js` 9/9 通过、`npm test` 1605/1605 通过、`git diff --check` 通过、docs validation 通过。它不调用真实 `search_memory` / `record_memory`，不读 `.jsonl`，不扫真实 memory，不写 durable memory/audit，不证明 memory recall reliable。状态仍为 `RC_NOT_READY_BLOCKED`。
