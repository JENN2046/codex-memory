...
2. 下一步只能继续处理 P66 记录的剩余 7 个 runtime gap / 16 个 A5 hard stop 中的本地可证明部分；若涉及 push/tag/release/deploy/config/watchdog/cutover/RC_READY，必须单独明确授权。P66.56 候选范围只能是 `rc_cutover_not_executed` 的本地 planning；P66.57 候选范围只能是 RC cutover acceptance fixture/test，不得执行 cutover gate、RC cutover、配置切换或声明 readiness。
