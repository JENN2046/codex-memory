...
- 当前候选：P66.42 ValidationAggregator recall isolation runtime proof gap planning。只能做 `recall_isolation_runtime_proof_not_executed` 的本地 docs/fixture/test 规划；不得扫描真实 memory/runtime stores、读取 diary/SQLite/vector/candidate/recall-audit、执行 runtime proof、写 durable state 或声明 readiness。
...
2. 下一步只能继续处理 P66 记录的剩余 7 个 runtime gap / 16 个 A5 hard stop 中的本地可证明部分；若涉及 push/tag/release/deploy/config/watchdog/cutover/RC_READY，必须单独明确授权。P66.42 候选范围只能是 `recall_isolation_runtime_proof_not_executed` 的本地规划；P66.43 候选范围只能是本地 fixture/test acceptance contract，不得扫描真实 memory/runtime stores。
