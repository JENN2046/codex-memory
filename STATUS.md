...
- 当前 CM-0560 search timeout boundary：已新增 [CM-0560 search memory timeout boundary](/A:/codex-memory/docs/CM-0560_SEARCH_MEMORY_TIMEOUT_BOUNDARY.md)。`search_memory` 现在有 app-level timeout wrapper，默认 `30000` ms，可通过 `CODEX_MEMORY_SEARCH_TIMEOUT_MS` / `searchMemoryTimeoutMs` 覆盖；timeout 通过 JSON-RPC error envelope 返回 `-32002` / `SEARCH_MEMORY_TIMEOUT`，并避免泄露 raw query。验证为 `node --check` 相关 source/test 通过、`node --test .\tests\mcp-contract.test.js` 8/8 通过、`node --test .\tests\phase-b-passive-recall.test.js` 4/4 通过。该修复不取消底层 in-flight recall promise，不证明 memory recall reliable；真实 `search_memory` validation、broad real-memory recall proof、durable write/audit、provider、config switch、public MCP expansion、push/tag/release/deploy/cutover 仍未执行。状态仍为 `RC_NOT_READY_BLOCKED`。
