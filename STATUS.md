...
- 当前结论：project_health strong；governance_health strong；current_truth `P22 security-fix tag created and pushed`；release_state `SECURITY_FIX_TAG_CREATED_RELEASE_DEPLOY_NOT_PERFORMED`；recommended_action `request explicit GitHub release approval for p22-rc-7fd17de if publication should continue`。旧 `p22-rc-806cc847` 已 superseded 且不得移动/复用；release、deploy、provider/config/migration/import-export/public MCP expansion 仍需单独显式批准。
