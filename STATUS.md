...
- 当前 CM-0561 search timeout cooperative abort boundary：已新增 [CM-0561 search memory cooperative abort boundary](/A:/codex-memory/docs/CM-0561_SEARCH_MEMORY_COOPERATIVE_ABORT_BOUNDARY.md)。CM-0561 在 CM-0560 timeout wrapper 基础上增加 `AbortController` / `AbortSignal` 传播，并在 app、recall pipeline、candidate generation 边界增加 cooperative abort checks；新增 mock MCP contract 证明 timeout 后不会继续调用 read-policy audit summary append。验证为 changed source/test `node --check` 通过、`node --test .\tests\mcp-contract.test.js` 9/9 通过、`node --test .\tests\phase-b-passive-recall.test.js` 4/4 通过。该修复仍不是 hard cancellation，不证明 memory recall reliable；真实 `search_memory` validation、broad real-memory recall proof、durable write/audit、provider、config switch、public MCP expansion、tag/release/deploy/cutover 仍未执行。状态仍为 `RC_NOT_READY_BLOCKED`。
