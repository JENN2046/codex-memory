...
- 当前结论：project_health strong；governance_health strong；current_truth `P22 security-fix GitHub prerelease created`；release_state `SECURITY_FIX_GITHUB_RELEASE_CREATED_DEPLOY_NOT_PERFORMED`；recommended_action `request explicit deploy approval only if deployment should continue`。旧 `p22-rc-806cc847` 已 superseded 且不得移动/复用；deploy、provider/config/migration/import-export/public MCP expansion 仍需单独显式批准。
