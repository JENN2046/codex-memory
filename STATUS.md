...
- Claude Code 本地 HTTP MCP 已添加到当前项目 local 配置：[CLAUDE_MCP_ACCEPTANCE.md](/A:/codex-memory/CLAUDE_MCP_ACCEPTANCE.md)。`claude mcp get/list` 显示 connected，直接 MCP `memory_overview` 调用成功；按用户要求改用 `deepseek-v4-pro` 后，模型侧工具调用仍受 API `ConnectionRefused` 阻塞，交互式 `/mcp` 待补验。
