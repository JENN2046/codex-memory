...
- 当前远端 `main` 已同步到 `76b1513`（`docs: plan workspace scope backfill review`）；PR #2 已按 superseded 说明关闭且未合并，远端分支 `codex/p1-vcp-memory-core-100-roadmap` 仍保留用于追溯。最新已验证基线：health `200`、compare `43/43 matched (0/0)`、rollback `43/43 rollback-ready (0/0)`、`npm test` `181/181`、`gate:mainline:strict` `ok`、`scope:acceptance` `ok`。`scope:backfill:dry-run` 当前显示 `450` 条记录中 `442` 条缺少 `workspace_id`，`mutated=false`，不自动回填；`real-query-suite` 已升级为 fixture-only baseline：`caseCount=5`、`placeholderCount=0`、`fixtureOnlyCount=5`、`realCount=5`。维护期验收快照：[maintenance-acceptance-2026-05-08.md](/A:/codex-memory/logs/maintenance-acceptance-2026-05-08.md)。
