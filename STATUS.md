...
- 当前结论：project_health strong；governance_health strong；current_truth `P22 security-fix tag-only approval requested but not approved`；release_state `SECURITY_FIX_RC_ARTIFACT_CREATED_TAG_RELEASE_DEPLOY_NOT_PERFORMED`；recommended_action `wait for explicit tag-only approval for p22-rc-7fd17de`。旧 `p22-rc-806cc847` 已 superseded 且不得移动/复用；tag、release、deploy、provider/config/migration/import-export/public MCP expansion 仍需单独显式批准。
