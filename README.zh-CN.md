...
它的核心不是重做一套记忆智能，而是把 Codex 对原生记忆的访问放进治理边界里：
...
- server exposed MCP tools：默认 read-only，只列出 `search_memory` / `memory_overview` / `audit_memory`
...
3. 先调用 `memory_overview` 看当前可披露的记忆概览。
4. 再调用 `search_memory` 做问题相关召回。
5. 必要时调用 `audit_memory` 查看受限审计投影。
...
| 能力 | 当前状态 | 说明 |
| --- | --- | --- |
| native read delegation | 已实现并真实 proof | 默认路径走 VCPToolBox native memory |
